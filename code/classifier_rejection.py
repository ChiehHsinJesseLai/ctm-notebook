...
class vpsde():
    ...
    def __init__(self, beta_min=0.1, beta_max=20., multiplier=1., cos_t_classifier=False,):
        """
        Initializes the VPSDE class with given parameters
        """
    ...
    def T(self):
        """
        Returns the constant time value.
        """
        return 1

    def compute_tau(self, std_wve_t, multiplier=-1.):
        """
        Computes the tau value based on the input standard deviation and multiplier
        """
        if multiplier == -1:
            if self.multiplier == 1.:
                tau = -self.beta_0 + th.sqrt(self.beta_0 ** 2 + 2.
                       * (self.beta_1 - self.beta_0) * th.log(1. + std_wve_t ** 2))
                tau /= self.beta_1 - self.beta_0
            elif self.multiplier == 2.:
                d = - th.log(1. + std_wve_t ** 2)
                in_ = (2 * (self.b ** 3) - 9 * self.a *self.b * self.c
                    + 27.* (self.a ** 2) * d) ** 2 - 4 * (((self.b ** 2)
                    - 3 * self.a * self.c) ** 3)
                ...
                tau = - self.b / (3. * self.a) - sign_plus * ((th.abs(plus) / 2.)
                **(1/3.)) / (3. * self.a)- sign_minus * ((th.abs(minus) / 2.)
                ** (1/3.)) / (3. * self.a)
        elif multiplier == 1.:
            tau = -self.beta_0 + th.sqrt(self.beta_0 ** 2 + 2. * (self.beta_1 - self.beta_0) *
                                          th.log(1. + std_wve_t ** 2))
            tau /= self.beta_1 - self.beta_0
        elif multiplier == 2.:
            d = - th.log(1. + std_wve_t ** 2)
            in_ = (2 * (self.b ** 3) - 9 * self.a * self.b
                 * self.c + 27. * (self.a ** 2) * d) ** 2 - 4 * (
            ...
            tau = - self.b / (3. * self.a) - sign_plus * ((th.abs(plus) / 2.) ** (1 / 3.)) / (
                    3. * self.a) - sign_minus * ((th.abs(minus) / 2.)
                    ** (1 / 3.)) / (3. * self.a)
        return tau

    def marginal_prob(self, t, multiplier=-1.):
        """
        Computes the mean and standard deviation for the marginal probability.
    ...
    def sampling_std(self, num_step):
        """
        Computes the standard deviation for sampling.

        """
        assert 1000 % num_step == 0
        ddim_timesteps = th.from_numpy(np.array(list
                        (range(0, 1000, 1000 // num_step)))[::-1].copy())
    ...
    def from_rescaled_t_to_original_std(self, rescaled_t):
        """
        Converts rescaled time to original standard deviation.
        """
        return th.exp(rescaled_t / 250.) - 1e-44

    def compute_t_cos_from_t_lin(self, t_lin):
        """
        Computes the cosine-transformed time from linear time.
        """
        sqrt_alpha_t_bar = th.exp(-0.25 * t_lin ** 2
        ...
        if multiplier == -1.:
            if self.multiplier == 1.:
                ...
                return ((self.beta_1 ** 0.5 - self.beta_0 ** 0.5) ** 2) * (t ** 3) / 3. \
                      + (self.beta_0 ** 0.5) * (self.beta_1 ** 0.5
                     - self.beta_0 ** 0.5)* (t ** 2) + self.beta_0 * t
        elif multiplier == 1.:
            return 0.5 * t ** 2 * (self.beta_1 - self.beta_0) + t * self.beta_0
        elif multiplier == 2.:
            return ((self.beta_1 ** 0.5 - self.beta_0 ** 0.5) ** 2) * (t ** 3) / 3. \
...
def get_classifier_guidance(classifier, vpsde, unnormalized_input,
    std_wve_t, img_resolution, class_labels, log_prob=False):
    mean_vp_tau, _, tau = vpsde.transform_unnormalized_wve_to_normalized_vp(std_wve_t, std_out=True)
    ...
    with th.no_grad():
        ...
        logits = classifier(x_, timesteps=tau)
        log_probs = F.log_softmax(logits, dim=-1)
        selected = log_probs[range(len(logits)), class_labels.view(-1)]
        if log_prob:
            return selected
        classifier_guidance = th.autograd.grad(outputs=selected.sum(),
...
def main():
    ...
    classifier = create_classifier(**args_to_dict(args,
                list(classifier_defaults().keys()) + ['image_size']))
    ...
    if os.path.exists(os.path.join(args.out_dir, 'classifier_rejection_v3.pickle')):
        ...
        sample_dirs = glob.glob(os.path.join(args.out_dir,
                    'ctm_exact_sampler_1_steps_028000_itrs_0.999_ema_/*.npz'))
        log_probs = []
        all_classes = []
        for k in tqdm(range(len(sample_dirs))):
            ...
            log_prob = get_classifier_guidance(classifier, vpsde_, samples,
                       0.002 * s_in, args.image_size,
                       classes_th, log_prob=True).cpu().detach().numpy().reshape(-1)
            log_probs.extend(log_prob)
            all_classes.extend(classes)
        log_probs_all = {i: {} for i in range(1000)}
        for k in range(1000):
            log_probs_all[k] = {i: log_probs[i] for i in
                        range(len(log_probs)) if all_classes[i] == k}
        with open(os.path.join(args.out_dir, 'classifier_rejection_v3.pickle'), 'wb') as handle:
            pickle.dump({'log_probs_all': log_probs_all,
            'log_probs': log_probs, 'all_classes': all_classes,
            'sample_dirs': sample_dirs}, handle)
        import sys
        sys.exit()
    ...
    for k in range(1000):
        top_50k.extend(
            list(dict(sorted(dict(list(log_probs_all[k].items())
            [:samples_per_class]).items(), key=lambda item: item[1], reverse=True)).keys())[:50])
    ...
    for sample_dir in sample_dirs:
        ...
        num += samples.shape[0]
    samples_top = np.concatenate(samples_top)
    n = 1
    ...
    os.makedirs(os.path.join(args.out_dir,
            f'ctm_exact_sampler_1_steps_028000_itrs_0.999_ema_single_npz_{args.rejection_ratio}_'),
            exist_ok=True)
    np.savez(os.path.join(args.out_dir, f'ctm_exact_sampler_1_steps_028000_itrs_0.999_ema_single_npz_{args.rejection_ratio}_/sorted_samples'),
    samples_top)
    dist.barrier()
