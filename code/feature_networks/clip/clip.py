...
def _download(url: str, root: str = os.path.expanduser("~/.cache/clip")):
    ...
    If the file already exists and its SHA256 checksum matches, it is not re-downloaded.
    If the checksum does not match, the file is re-downloaded.
    """
    ...
    with urllib.request.urlopen(url) as source, open(download_target, "wb") as output:
        with tqdm(total=int(source.info().get("Content-Length")),
                  ncols=80, unit='iB', unit_scale=True) as loop:
            while True:
...
def _transform(n_px):
    ...
    The transform includes resizing, center cropping, converting to RGB, 
...
class ClipHead(nn.Module):
    """
    A neural network module that uses a CLIP model to calculate loss
    based on text and image features.
    """
    def __init__(self, prompt, device='cpu'):
        """
        Initializes the ClipHead module with a given prompt and device.
        """
        super().__init__()
    ...
    def calc_loss(self, features):
        """
        Calculates the loss based on the cosine similarity between text and image features.
        """
