...
def calculate_inception_stats_npz(image_path, num_samples=50000,
                                  batch_size=250, device=torch.device('cuda'),):
    """
    Calculates the Inception statistics (mean and covariance) for a set of images.
    """
...
def calculate_fid_from_inception_stats(mu, sigma, mu_ref, sigma_ref):
    """
    Calculate the Frechet Inception Distance (FID) between two sets of statistics.
    """
...
@click.command()
@click.option('--images', 'image_path', help='Path to the images',
              metavar='PATH|ZIP',              type=str, required=True)
@click.option('--ref', 'ref_path',      help='Dataset reference statistics ',
               metavar='NPZ|URL',    type=str, required=True)
...
def main(image_path, ref_path, num_samples, batch_size, device):
    ...
    try:
        ...
        ref_mu, ref_sigma = calculate_inception_stats_npz(image_path=ref_path,
                                                num_samples=num_samples, batch_size=batch_size,
                                                device=device)

    mu, sigma = calculate_inception_stats_npz(image_path=image_path,
