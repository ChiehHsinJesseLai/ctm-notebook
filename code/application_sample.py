...
from cm.sample_util import application_sample
...
def main():
    """
    perform the model,diffusion generation
    """
    ...
    if args.sampler in ['multistep', 'exact', 'cm_multistep']:
        out_dir = os.path.join(
                     args.out_dir,
                     f'{args.training_mode}_{args.sampler}_sampler_{args.sampling_steps}_steps_'
                     f'{step}_itrs_{ema}_ema_{"".join([str(i) for i in ts])}'
                     )
    elif args.sampler in ["gamma"]:
        out_dir = os.path.join(args.out_dir,
                  f'{args.training_mode}_{args.sampler}_sampler_{args.sampling_steps}_steps_{step}_itrs_{ema}_ema_{"".join([str(i) for i in ts])}_ind1_{args.ind_1}_ind2_{args.ind_2}')
        ...
        out_dir = os.path.join(args.out_dir,f'{args.training_mode}_{args.sampler}_sampler_{args.sampling_steps}_steps_{step}_itrs_{ema}_ema_{args.gamma}_{ts[0]},{ts[1]}_numgrad_{args.num_gradient_descent}_scale_{args.scale}_{ref}_{r}')
    ...
    while itr * args.batch_size < args.eval_num_samples:
        ...
        if args.class_cond:
            if args.train_classes >= 0:
                classes = th.ones(size=(args.batch_size,), device=dist_util.dev(),
                          dtype=int) * int(args.train_classes)
            elif args.train_classes == -2:
                classes = [0, 1, 9, 11, 29, 31, 33, 55, 76, 89, 90,
                          130,207, 250, 279, 281, 291, 323, 386, 387,
                          388, 417, 562,614, 759, 789, 800, 812, 848,
                          933, 973, 980]
                assert args.batch_size % len(classes) == 0
                classes = th.tensor([x for x in classes
                          for _ in range(args.batch_size // len(classes))], device=dist_util.dev())
            else:
                classes = th.randint(
        ...
        images = th.from_numpy(np.array(image)).repeat(args.batch_size,1,1,1).to(
                 dist_util.dev()).permute((0,3,1,2))
        images = images / 127.5 - 1.
        with th.no_grad():
            x_out,_ = application_sample(
                images=images,
                diffusion=diffusion,
                model=model,
                ...
                clip_denoised=False if args.data_name in ['church']
                            else True if args.training_mode=='edm' else args.clip_denoised,
        ...
        if dist.get_rank() == 0:
            ...
            if args.save_format == 'npz':
                if args.class_cond:
                    ...
                    f"sample_out_{args.gamma}_{itr}.npz"), 
                    sample.numpy(), classes.cpu().detach().numpy())
                else:
                    np.savez(os.path.join(out_dir,
                    f"stroke_sample_out_{args.gamma}_{itr}_{ts[0]},{ts[1]}_numgrad_{args.num_gradient_descent}_scale_{args.scale}.npz"),sample.numpy())
