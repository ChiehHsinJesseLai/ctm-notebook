...
def main():
    """
    Set up and run the sampling loop
    """
    ...
    def sample_generator(ts):
        """
        Generate samples based on the given timestep sequence
        """
        logger.log("sampling...")
        all_images = []
        all_labels = []
    ...
    def get_fid(p, begin=(0,), end=(args.steps - 1,)):
        """
        Calculate FID and IS for the given parameters
        """
        _, preds = sample_generator(begin + (p,) + end)
    ...
    def ternary_search(before=(0,), after=(17,)):
        """
        Perform ternary search to find the optimal parameter
        """
        left = before[-1]
        right = after[0]
        is_root = dist.get_rank() == 0
...
def create_argparser():
    """
    Create an argument parser with default value
    """
    defaults = dict(
        begin="0",
        end="39",
