...
def get_generator(generator, num_samples=0, seed=0):
    """
    Get a generator instance based on the specified type.
    """
    if generator == "dummy":
...
class DummyGenerator:
    """
    Dummy generator for generating random numbers using PyTorch.
    """
    ...
    def randn_like(self, *args, **kwargs):
        """
        Generate random numbers like a given tensor.
        """
...
class DeterministicGenerator:
    ...
    on batch_size or mpi_machines
    Uses a single rng and samples num_samples sized randomness and s
    ubsamples the current indices
    """
    ...
    def get_global_size_and_indices(self, size):
        """
        Get the global size and indices for the given size.
        """
    ...
    def get_generator(self, device):
        """
        Get the appropriate generator for the given device.
        """
    ...
    def randint(self, low, high, size, dtype=th.long, device="cpu"):
        ...
        global_size, indices = self.get_global_size_and_indices(size)
        generator = self.get_generator(device)
        return th.randint(low, high, generator=generator,
                          size=global_size, dtype=dtype,
    ...
    def set_done_samples(self, done_samples):
        """
        Set the number of done samples.
...
class DeterministicIndividualGenerator:
    ...
    def get_size_and_indices(self, size):
        """
        Get the size and indices for the given size.
        """
        indices = th.arange(
            self.done_samples + self.rank,
            self.done_samples + self.world_size * int(size[0]),
