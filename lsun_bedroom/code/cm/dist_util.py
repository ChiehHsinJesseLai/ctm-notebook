...
def setup_dist_without_MPI(device_id):
    ...
    if backend == 'gloo':
        ...
        dist.init_process_group(backend='nccl', timeout=timedelta(seconds=5600), world_size=world_size, rank=rank)
    '''while flag:
        port = str(65535 - num)
        os.environ["MASTER_PORT"] = port#str(port[0])
...
def dev():
    ...
    _device = th.device("cuda", dist.get_rank()) if th.cuda.is_available() else th.device("cpu")
    return _device
