...
from feature_networks.constants import (NORMALIZED_INCEPTION,
                                        NORMALIZED_IMAGENET,
                                        NORMALIZED_CLIP,
                                        VITS)
...
def get_backbone_normstats(backbone):
    """
    Get normalization statistics for the given backbone
    """
...
def _make_scratch_ccm(scratch, in_channels, cout, expand=False):
    """
    Create scratch CCM layers
    """
    # shapes
    out_channels = [cout, cout*2, cout*4, cout*8] if expand else [cout]*4

    scratch.layer0_ccm = nn.Conv2d(in_channels[0],
                                   out_channels[0],
                                   kernel_size=1,
                                   stride=1,
                                   padding=0,
    ...
    scratch.layer1_ccm = nn.Conv2d(in_channels[1],
                                   ...
                                   padding=0,
                                   bias=True)
    scratch.layer2_ccm = nn.Conv2d(in_channels[2],
                                   out_channels[2],
                                   kernel_size=1,
                                   stride=1,
                                   padding=0,
                                   bias=True)
    scratch.layer3_ccm = nn.Conv2d(in_channels[3],
                                   out_channels[3],
                                   kernel_size=1,
                                   stride=1,
                                   padding=0,
...
def _make_scratch_csm(scratch, in_channels, cout, expand):
    """
    Create scratch CSM layers
    """
    scratch.layer3_csm = FeatureFusionBlock(in_channels[3],
                                            nn.ReLU(False),
                                            expand=expand,
                                            lowest=True)
    scratch.layer2_csm = FeatureFusionBlock(in_channels[2],
                                            nn.ReLU(False),
                                            expand=expand)
    scratch.layer1_csm = FeatureFusionBlock(in_channels[1],
                                            nn.ReLU(False),
                                            expand=expand)
...
def _make_projector(im_res, backbone, cout, proj_type, expand=False):
    """
    Create a projector with the given parameters
    """
...
class F_RandomProj(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initialize the F_RandomProj module with the given parameters.
        """
        super().__init__()
        ...
        self.pretrained, self.scratch = _make_projector(im_res=im_res,
    ...
    def forward(self, x):
        """
        Forward pass to predict feature maps based on the backbone and projection type
        """
