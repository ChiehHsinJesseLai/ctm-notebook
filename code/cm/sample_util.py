...
def get_t(ind, start_scales=40):
    """
    Calculate the time step 't' based on the given index and start scales.
    """
    rho = 7.
    sigma_max = 80.
    sigma_min = 0.002
...
def karras_sample(
    ...
    """
    Perform Karras sampling using the specified parameters.
    """
    ...
    def denoiser(x_t, t, s=th.ones(x_T.shape[0], device=device)):
        """
        Applies denoising to the input tensor at a given 
        timestep with optional scaling.
        """
        denoised, G_theta = diffusion.get_denoised_and_G(model, x_t, t, s, ctm,
                                                         teacher, **model_kwargs)
...
def sample_gamma_multistep_(
    ...
    for i in indices[:-1]:
        ...
        if i < len(indices) - 2:
            ...
            x = denoised + std * generator.randn_like(denoised)
        else:
            x = denoised
...
def sample_gamma_(
    ...
    """Implements Algorithm 2 (Heun steps) from Karras et al. (2022)."""
    assert ind_1 >= ind_2
    print(f"{get_t(0, steps)} -> "
        f"{get_t(ind_1, steps)} -> "
        f"{get_t(ind_2, steps)} -> "
        f"{get_t(39, steps)}")
    ones = th.ones(x.shape[0], device=x.device)
    denoised = denoiser(x, get_t(0, steps) * ones, s=get_t(ind_1, steps) * ones)
    denoised = denoised + th.randn_like(denoised) * np.sqrt(get_t(
        ind_2, steps) ** 2 - get_t(ind_1, steps) ** 2)
    x = denoiser(denoised, get_t(ind_2, steps) * ones,
                 s=get_t(39, steps) * ones)
    ...
    for i in indices[:-1]:
        ...
        if classifier != None and i < len(indices) - 2:
            denoised = denoised + cg_scale * (s * s_in)[:,None,None,None] * get_classifier_guidance(
                       classifier, vpsde_, denoised, s * s_in, x.shape[-2], class_labels)
        if i < len(indices) - 2:
            if edm_style:
            ...
            x = denoised
    log_prob = get_classifier_guidance(classifier, vpsde_, x,
                            0.002 * s_in, x.shape[-2], class_labels, log_prob=True)
...
def sample_gamma(
    ...
    for _ in range(langevin_steps):
        if cg_scale != 0.0:
            cg = get_classifier_guidance(classifier, vpsde_, G_s,
                                    get_t(ind_1, steps) * ones, x.shape[-2], class_labels)
        else:
            cg = 0.
    ...
    G_s = G_s + generator.randn_like(G_s) * \
      np.sqrt(get_t(ind_2, steps) ** 2 - \
              get_t(ind_1, steps) ** 2)
...
def stochastic_iterative_sampler(
    ...
    """
    Performs stochastic iterative sampling using the provided distiller and generator.
    """
    t_max_rho = t_max ** (1 / rho)
    t_min_rho = t_min ** (1 / rho)
    s_in = x.new_ones([x.shape[0]])
...
def sample_progdist(
    ...
    """
    Samples progressively using the provided denoiser and sigma values.
    """
    s_in = x.new_ones([x.shape[0]])
...
def application_sample(
    ...
    """
    Samples images using the specified diffusion model and parameters.
    """
    if generator is None:
        if generator_type == 'dummy':
            generator = get_generator("dummy")
    ...
    def denoiser(x_t, t, s=th.ones(shape[0], device=device)):
        ...
        timestep with optional scaling.
        """
        denoised, G_theta = diffusion.get_denoised_and_G(model, x_t, t, s,
                                                         ctm, teacher, **model_kwargs)
    ...
    if 'inpainting' in sampler:
        ...
        sampler_args = {'classifier': classifier,
                        'num_gradient_descent': num_gradient_descent,
                        'scale': scale,
                        'out_dir': out_dir}
    else:
        sampler_args = {}
...
def iterative_colorization(
    ...
    """
    Performs iterative colorization using the provided distiller and generator.
    """
    def obtain_orthogonal_matrix():
        """
        Obtains an orthogonal matrix for color transformation.
        """
    ...
    def replacement(x0, x1):
        """
        Replaces parts of x0 with x1 using a predefined mask and orthogonal matrix.
        """
...
def iterative_inpainting(
    ...
    """
    Performs iterative inpainting using the provided distiller and generator.
    """
    ...
    def replacement(x0, x1, flip=False):
        """
        Replaces parts of x0 with x1 using a predefined mask and optional flipping.
        """
        if flip:
    ...
    for i in range(len(ts) - 1):
        ...
        if gamma == 1.0:
            ...
            x0 = replacement(masked_images + s * generator.randn_like(masked_images), x0, flip=flip)
...
def iterative_superres(
    ...
    """
    Perform iterative super-resolution on images using a distiller model.
    """
    patch_size = 8

    def obtain_orthogonal_matrix():
        """
        Generate an orthogonal matrix with a specific structure.
        """
    ...
    def replacement(x0, x1):
        """
        Replace patches of x0 with patches from x1 using an orthogonal transformation.
        """
        x0_flatten = (
    ...
    def average_image_patches(x):
        """
        Average the pixel values within each patch of the image.
        """
        x_flatten = (
    ...
    for i in range(len(ts) - 1):
        ...
        if gamma == 1.0:
            ...
            x0 = replacement(low_res_images + s * generator.randn_like(low_res_images), x0)
            x = x0 + generator.randn_like(x) * next_t * gamma
...
def iterative_stroke_painting(
    ...
    """
    Perform iterative stroke painting on images using a distiller model.
    """
    ...
    if gamma == 1.0:
            ...
            with th.enable_grad():
                for k in range(num_gradient_descent):
                    ...
                    np.savez(os.path.join(out_dir,
                      f"stroke_sample_{str(t)[:6]},"
                      f"{str(next_t)[:6]}_{str(s)[:6]}_"
                      f"numgrad_{num_gradient_descent}_"
                      f"scale_{scale}_{k}th.npz"),
                      sample.cpu().detach().numpy())
                    nrow = int(np.sqrt(stroke_images.shape[0]))
                    image_grid = make_grid((th.clamp(stroke_images, -1., 1.) + 1.) / 2.,
                                           nrow, padding=2)
                    with bf.BlobFile(os.path.join(out_dir,
                              f"stroke_sample_{str(t)[:6]},"
                              f"{str(next_t)[:6]}_{str(s)[:6]}_"
                              f"numgrad_{num_gradient_descent}_"
                              f"scale_{scale}_{k}th.png"),"wb") as fout:
                    ...
                    mean_vp_tau, tau = vpsde_.transform_unnormalized_wve_to_normalized_vp(
                                                            th.tensor([s],
                                                            device=x.device))
                    fake = mean_vp_tau[:, None, None, None] * x_
                    true = mean_vp_tau[:, None, None, None] * (
                        reference_images + s * generator.randn_like(x))
                    tau = tau.reshape(-1) * s_in
                    fake_feat = classifier(fake, timesteps=tau, feature=True)
                    true_feat = classifier(true, timesteps=tau, feature=True)
                    feat_diff = (fake_feat - true_feat) ** 2
                    print("loss: ", feat_diff.sum().item())
                    grad = th.autograd.grad(outputs=feat_diff.sum(
                    ...
                    with th.no_grad():
                        ...
                        log_prob = score - scale * (
                            th.norm(score.reshape(score.shape[0], -1), dim=1) /
                            th.norm(grad.reshape(grad.shape[0], -1), dim=1)
                            )[:, None, None, None] * grad
                        grad_norm = th.norm(log_prob.reshape(log_prob.shape[0], -1), dim=-1).mean()
                        noise_norm = th.norm(noise.reshape(noise.shape[0], -1), dim=-1).mean()
                        step_size = (0.1 * noise_norm / grad_norm) ** 2 * 2
                        x = x + step_size * log_prob + th.sqrt(
                            2. * step_size) * generator.randn_like(x)
        stroke_images = th.clamp(distiller(x, s * s_in, s=0.002 * s_in)[1], -1.0, 1.0)
        sample = ((stroke_images + 1) * 127.5).clamp(0, 255).to(th.uint8)
        sample = sample.permute(0, 2, 3, 1)
        sample = sample.contiguous()
        np.savez(os.path.join(out_dir,
            ...
            f"{str(next_t)[:6]}_{str(s)[:6]}_"
            f"numgrad_{num_gradient_descent}_"
            f"scale_{scale}_{num_gradient_descent}th.npz"),
            sample.cpu().detach().numpy())

        nrow = int(np.sqrt(stroke_images.shape[0]))
        image_grid = make_grid((th.clamp(
            stroke_images, -1., 1.) + 1.) / 2., nrow, padding=2)
        with bf.BlobFile(os.path.join( out_dir,
            f"stroke_sample_{str(t)[:6]},"
            f"{str(next_t)[:6]}_{str(s)[:6]}_"
            f"numgrad_{num_gradient_descent}_"
            f"scale_{scale}_{num_gradient_descent}th.png"), "wb") as fout:        
            save_image(image_grid, fout)
    ...
    np.savez(os.path.join(out_dir,
        f"stroke_sample_{str(t)[:6]},"
        f"{str(next_t)[:6]}_{str(s if gamma != 1.0 else 0)[:6]}_"
        f"numgrad_{num_gradient_descent}_"
        f"scale_{scale}_{num_gradient_descent}th_after_diffusion.npz"
        ),sample.cpu().detach().numpy())

    nrow = int(np.sqrt(stroke_images.shape[0]))
    image_grid = make_grid((th.clamp(stroke_images,
                                     -1., 1.) + 1.) / 2., nrow, padding=2)
    with bf.BlobFile(os.path.join(out_dir,
        f"stroke_sample_{str(t)[:6]},"
        f"{str(next_t)[:6]}_{str(s if gamma != 1.0 else 0)[:6]}_"
        f"numgrad_{num_gradient_descent}_"
        f"scale_{scale}_{num_gradient_descent}th_after_diffusion.png"),"wb") as fout:
        save_image(image_grid, fout)
