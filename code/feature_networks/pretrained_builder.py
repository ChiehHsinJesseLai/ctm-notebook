...
def _feature_splitter(model, idcs):
    """
    Splits the features of a model into layers based on provided indices.
    """
...
def _make_resnet(model):
    """
    Creates a pretrained ResNet model by organizing its layers.
    """
    pretrained = nn.Module()
...
def _make_regnet(model):
    """
    Creates a pretrained RegNet model by organizing its layers.
    """
    pretrained = nn.Module()
    pretrained.layer0 = nn.Sequential(
        model.stem, model.s1
...
def _make_nfnet(model):
    """
    Creates a pretrained NFNet model by organizing its layers.
    """
    pretrained = nn.Module()
    pretrained.layer0 = nn.Sequential(
...
def _make_resnet_v2(model):
    """
    Creates a pretrained ResNet v2 model by organizing its layers.
    """
    pretrained = nn.Module()
...
def _make_resnet_clip(model):
    """
    Creates a pretrained ResNet model for CLIP by organizing its layers.
    """
    pretrained = nn.Module()
...
def _make_densenet(model):
    """
    Creates a pretrained DenseNet model by organizing its layers.
    """
    pretrained = nn.Module()
...
def _make_shufflenet(model):
    """
    Creates a pretrained ShuffleNet model by organizing its layers.
    """
    pretrained = nn.Module()
...
def _make_cspresnet(model):
    """
    Creates a pretrained CSPResNet model by organizing its layers.
    """
    pretrained = nn.Module()
    pretrained.layer0 = nn.Sequential(model.stem, model.stages[0])
    pretrained.layer1 = model.stages[1]
...
def _make_efficientnet(model):
    """
    Creates a pretrained EfficientNet model by organizing its layers.
    """
    pretrained = nn.Module()
    pretrained.layer0 = nn.Sequential(
...
def _make_ghostnet(model):
    """
    Creates a pretrained GhostNet model by organizing its layers.
    """
    pretrained = nn.Module()
    pretrained.layer0 = nn.Sequential(
...
def _make_vit(model, name):
    """
    Creates a pretrained Vision Transformer (ViT) model by organizing 
    its layers based on the model name.
    """
    if 'tiny' in name:
...
def calc_dims(pretrained, is_vit=False):
    """
    Calculates the dimensions of the output features for each layer of the pretrained model.
    """
    dims = []
    inp_res = 256
...
def _make_pretrained(backbone, verbose=False):
    """
    Creates a pretrained model by selecting and organizing its 
    layers based on the specified backbone
    """
