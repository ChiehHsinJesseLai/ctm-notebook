...
class ScheduleSampler(ABC):
    ...
    def sample_s(self, args, batch_size, device,
                 indices, num_heun_step=1,
                 time_continuous=False, N=40):
        """
        Samples new indices based on the given strategy 
        and parameters.
        """
        if time_continuous:
            new_indices = th.from_numpy(np.random.rand(indices.shape[0]
                        )).to(indices.device) * (1. - indices - num_heun_step)\
...
class UniformSampler(ScheduleSampler):
    """
    A sampler that generates uniform weights for a given number of timesteps.
    """
    ...
    def weights(self, num_heun_step=1):
        """
        Generates uniform weights adjusted by the number of Heun steps.
        """
...
class LossAwareSampler(ScheduleSampler):
    """
    Class for sampling with loss-aware reweighting.
    """
...
class LossSecondMomentResampler(LossAwareSampler):
    """
    A sampler that adjusts weights based on the second moment of loss history.
    """
    ...
    def weights(self):
        """
        Calculates and returns the weights based on the second moment of the loss history.
        """
    ...
    def update_with_all_losses(self, ts, losses):
        """
        Updates the loss history with new loss values.
        """
    ...
    def _warmed_up(self):
        """
        Checks if the loss history is fully populated.
...
class LogNormalSampler:
    """
    A sampler that generates samples from a log-normal distribution.

    """
    def __init__(self, p_mean=-1.2, p_std=1.2, even=False):
        """
        Initializes the LogNormalSampler with the given parameters.
        """
        self.p_mean = p_mean
        self.p_std = p_std
        self.even = even
...
class HalfLogNormalHalfUniformSampler:
    """
    A sampler that generates samples from a combination of log-normal and uniform distributions.
    """
    ...
    def get_t(self, t):
        """
        Transforms the input t based on the sigma parameters.
        """
        t = self.args.sigma_max ** (1 / self.args.rho) + t * (
                self.args.sigma_min ** (
                1 / self.args.rho) - self.args.sigma_max ** (
                    1 / self.args.rho))
        t = t ** self.args.rho
        return t
    ...
    def sample(self, bs, device, num_heun_step=1):
        """
        Samples from the combined log-normal and 
        uniform distributions.
        """
        log_sigmas = self.p_mean + self.p_std * th.randn(bs//2, device=device)
        sigmas = th.exp(log_sigmas)
        t = th.rand(bs - bs//2, device=device) * self.args.diffusion_mult
