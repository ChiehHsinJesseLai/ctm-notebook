...
def get_weightings(weight_schedule, snrs, sigma_data, t, s, schedule_multiplier=None,):
    """
    Computes weightings based on the specified weight schedule.
    """
...
class KarrasDenoiser:
    """
    A denoiser class implementing various noise schedules and scaling methods for diffusion models.
    """
    def __init__(
        self,
        args,
    ...
    def get_snr(self, sigmas):
        """
        Computes the signal-to-noise ratio (SNR) for given sigmas.
        """
        return sigmas**-2

    def get_c_in(self, sigma):
        """
        Computes the input scaling factor for a given sigma.
        """
        return 1 / (sigma**2 + self.args.sigma_data**2) ** 0.5

    def get_inner_scalings(self, t, inner_parametrization='no'):
        """
        Computes the inner scaling factors for a given timestep and parametrization.
        """
    ...
    def get_outer_scalings(self, t, s=None, outer_parametrization='euler'):
        """
        Computes the outer scaling factors for a given timestep and parametrization.
        """
        if outer_parametrization == 'euler':
            ...
            c_skip = s / (t + 1.) + \
                     (((s - self.args.sigma_min
                        ) ** 2 + self.args.sigma_data ** 2) /
                      ((t - self.args.sigma_min
                        ) ** 2 + self.args.sigma_data ** 2)).sqrt() / (t + 1.)
        c_out = 1. - s / t
        return c_skip, c_out

    def get_edm_scalings(self, sigma):
        """
        Computes the EDM scaling factors for a given sigma.
        """
    ...
    def get_cm_scalings(self, sigma):
        """
        Computes the CM scaling factors for a given sigma.
        """
    ...
    def calculate_adaptive_weight(self, loss1, loss2, last_layer=None):
        """
        Calculates an adaptive weight based on the gradients of two losses.
        """
    ...
    def adopt_weight(self, weight, global_step, threshold=0, value=0.):
        """
        Adopts a weight based on the global step and a threshold.
    ...
    def get_t(self, ind):
        """
        Computes the timestep for a given index.
        """
        if self.args.time_continuous:
            t = self.args.sigma_max ** (1 / self.args.rho) + ind * (
                self.args.sigma_min ** (1 / self.args.rho
                ) - self.args.sigma_max ** (1 / self.args.rho))
            t = t ** self.args.rho
        else:
    ...
    def get_num_heun_step(self, start_scales=-1, num_heun_step=-1,
                        num_heun_step_random=None, heun_step_strategy='',
                        time_continuous=None):
        """
        Computes the number of Heun steps based on various parameters.
        """
        if start_scales == -1:
        ...
        if num_heun_step_random:
            if time_continuous:
                ...
                if heun_step_strategy == 'uniform':
                    ...
                    p = np.array([i ** self.args.heun_step_multiplier
                                for i in range(1,1+num_heun_step)])
                    p = p / sum(p)
    ...
    def get_gan_time(self, x_start, noise, x_t, t, t_dt, s,
                    indices, num_heun_step, gan_num_heun_step):
        """
        Computes the GAN time steps and updates the necessary 
        variables based on the given parameters.
        """
        if gan_num_heun_step != -1:
            indices, _ = self.schedule_sampler.sample_t(x_start.shape[0],
                        x_start.device, gan_num_heun_step,
                        self.args.time_continuous)
            new_indices = self.schedule_sampler.sample_s(self.args,
                                x_start.shape[0], x_start.device,
                                indices,
    ...
    @th.no_grad()
    def heun_solver(self, target_model, x,
                    ind, dims, t, t_dt, ctm=True,
                    num_step=1, **model_kwargs):
        """
        Solves the Heun's method for the given model and parameters.
        """
        with th.no_grad():
            if self.args.self_learn:
                if self.args.self_learn_iterative:
                    for k in range(num_step):
                        ...
                        _, x = self.get_denoised_and_G(target_model,
                                x, t, s=t2, ctm=ctm, **model_kwargs)
                else:
                    _, x = self.get_denoised_and_G(target_model,
                                                   x, t, s=t_dt, ctm=ctm, **model_kwargs)
            else:
                for k in range(num_step):
                    t = self.get_t(ind + k)
                    denoised, _ = self.get_denoised_and_G(self.teacher_model,
                                   x, t, None, ctm=False, teacher=True, **model_kwargs)
                    d = (x - denoised) / append_dims(t, dims)
                    t2 = self.get_t(ind + k + 1)
                    x_phi_ODE_1st = x + d * append_dims(t2 - t, dims)
                    denoised2, _ = self.get_denoised_and_G(self.teacher_model,
                                   x_phi_ODE_1st, t2, None, ctm=False, teacher=True, **model_kwargs)
                    next_d = (x_phi_ODE_1st - denoised2) / append_dims(t2, dims)
                    x_phi_ODE_2nd = x + (d + next_d) * append_dims((t2 - t) / 2, dims)
                    x = x_phi_ODE_2nd
            return x

    def get_gan_fake(self, estimate, x_t, t,
                     t_dt, s, model, target_model, ctm,
                     step, **model_kwargs):
        """
        Generates a fake sample using the GAN model based on the given parameters.
        """
        if self.args.gan_fake_outer_type == 'no':
            _, fake = self.get_denoised_and_G(model, x_t,t,
                            s=th.ones_like(s) * self.args.sigma_min,
            ...
            if (self.args.gan_fake_outer_type == self.args.ctm_estimate_outer_type) \
                ...
                fake = self.get_ctm_estimate(x_t, t, t_dt, s, model,target_model, ctm,
                                    ...
                                    target_matching=self.args.gan_target_matching,
                                    **model_kwargs)
    ...
    @th.no_grad()
    def get_gan_real(self, x_start, x_t, t, t_dt,
                     s, indices, dims, num_heun_step,
                     model, target_model, ctm, step, **model_kwargs):
        """
        Generates a real sample using the GAN model based on the given parameters.
        ...
        with th.no_grad():
            if self.args.gan_real_free:
                ...
                x_t_dt = self.heun_solver(target_model, x_t,
                                          indices, dims, t, t_dt,
                                          ctm=ctm, num_step=num_heun_step,
                                          **model_kwargs).detach()
    ...
    def get_ctm_estimate(self, x_t, t, t_dt, s,
                         model, target_model,ctm,
                         outer_type, inner_type,
                         target_matching, **model_kwargs):
        """
        Estimates the CTM (Continuous Time Model) based on the given parameters.
        """
        ...
        if inner_type == 'model':
            ...
            with th.no_grad():
                _, estimate = self.get_denoised_and_G(model,
                                            x_t, t, s=s,
                                            ctm=ctm, **model_kwargs)
        elif inner_type == 'target_model_sg':
            with th.no_grad():
                _, estimate = self.get_denoised_and_G(target_model,x_t,
                                                      t, s=s, ctm=ctm,
                                                      **model_kwargs)
        else:
            raise NotImplementedError
        if self.args.training_mode == 'ctm':
            if outer_type == 'model':
                _, estimate = self.get_denoised_and_G(model, estimate,s,
                                        s=th.ones_like(s) * self.args.sigma_min,
                                        ctm=ctm, **model_kwargs)
            elif outer_type == 'target_model_sg':
                _, estimate = self.get_denoised_and_G(target_model, estimate,
                                                s, s=th.ones_like(s) * self.args.sigma_min,
                                                ctm=ctm, **model_kwargs)
            else:
                raise NotImplementedError
        return estimate
    @th.no_grad()
    def get_ctm_target(self, x_t_dt, t_dt,s,
                       model, target_model,ctm,
                       inner_type, **model_kwargs):
        """
        Generates the CTM (Continuous Time Model) target based on the given parameters.
        """
        if self.args.large_log:
            print("CTM target inner type, ctm: ", inner_type, ctm)
        with th.no_grad():
            if inner_type == 'model_sg':
                _, target = self.get_denoised_and_G(model, x_t_dt,
                                                    t_dt, s=s, ctm=ctm,
                                                    **model_kwargs)
            elif inner_type == 'target_model_sg':
                _, target = self.get_denoised_and_G(target_model, x_t_dt,t_dt,
                                                    s=s, ctm=ctm, **model_kwargs)
            elif inner_type == 'no':
                target = x_t_dt
                s = t_dt
            else:
                raise NotImplementedError
            if self.args.training_mode == 'ctm':
                _, target = self.get_denoised_and_G(target_model, target,s,
                                            s=th.ones_like(s) * self.args.sigma_min,
                                            ctm=ctm, **model_kwargs)
    ...
    def get_denoised(self, g_theta, x_t ,t):
        """
        Computes the denoised tensor based on the given parameters
        and the specified outer parametrization.
        """
        if self.args.outer_parametrization.lower() == 'euler':
            ...
            denoised = g_theta + x_t - append_dims(t / (t + 1.) * (1. + (
                        t - self.args.sigma_min) /((
                        t - self.args.sigma_min) ** 2 + self.args.sigma_data ** 2)),
                        x_t.ndim) * x_t
        else:
    ...
    def get_denoised_and_G(self, model, x_t, t, s=None, ctm=False, teacher=False, **model_kwargs):
        """
        Computes the denoised tensor and G_theta based on the given parameters and model
        """
        ...
        if ctm:
            if self.args.target_subtract:
                with th.no_grad():
                    teacher_denoised = self.teacher_model(c_in * x_t,
                                        rescaled_t, s=None,
                                        teacher=True,**model_kwargs)
                if self.args.rescaling:
                    model_output = model_output * append_dims((
                                t ** 2 - s ** 2) ** 0.5 / t, x_t.ndim)
    ...
    def get_CTM_loss(self, estimate, target, weights, step):
        """
        Computes the CTM (Continuous Time Model) loss based on the given parameters.
        ...
        if self.args.loss_norm == 'lpips':
            ...
            for _, _ in self.feature_extractor.items():
                for fe in list(estimate_features[cnt].keys()):
                    norm_factor = th.sqrt(th.sum(estimate_features[cnt
                                    ][fe] ** 2, dim=1, keepdim=True))
                    est_feat = estimate_features[cnt][fe] / (norm_factor + 1e-10)
                    norm_factor = th.sqrt(th.sum(target_features[
                        cnt][fe] ** 2, dim=1, keepdim=True))
    ...
    def get_DSM_loss(self, model, x_start, model_kwargs, consistency_loss,
                           step, init_step):
        """
        Computes the DSM (Denoising Score Matching) loss based on the given parameters.
        """
        sigmas, denoising_weights = self.diffusion_schedule_sampler.sample(
                                    x_start.shape[0], dist_util.dev())
        noise = th.randn_like(x_start)
        dims = x_start.ndim
        x_t = x_start + noise * append_dims(sigmas, dims)
        denoised, _ = self.get_denoised_and_G(model, x_t, sigmas,
                                              s=sigmas, ctm=True,
                                              teacher=True, **model_kwargs)
        snrs = self.get_snr(sigmas)
        denoising_weights = append_dims(get_weightings(self.args.diffusion_weight_schedule,
                                        snrs, self.args.sigma_data, None, None), dims)
        ...
        if self.args.apply_adaptive_weight:
            if self.args.data_name.lower() == 'cifar10':
                balance_weight = self.calculate_adaptive_weight(consistency_loss.mean(),
                                                                denoising_loss.mean(),
    ...
    def get_GAN_loss(self, model, real=None, fake=None,
                     consistency_loss=None,
                     learn_generator=True, discriminator=None,
                     step=0, init_step=0, **model_kwargs):
        """
        Computes the GAN loss based on the given parameters.

        """
        if learn_generator:
            logits_fake = get_xl_feature(self.args, fake,
            ...
            if self.args.d_apply_adaptive_weight:
                ...
                d_weight = 1.
            discriminator_loss = self.adopt_weight(d_weight,
                                                   step,
                                                   threshold=init_step
                                                   + self.args.discriminator_start_itr) * g_loss
        else:
            logits_fake, logits_real = get_xl_feature(self.args, fake.detach(),
                                target=real.detach(),
                                feature_extractor=self.discriminator_feature_extractor,
                                discriminator=discriminator,
                                step=step, **model_kwargs)
            loss_Dgen = sum([(F.relu(th.ones_like(l) + l)).mean(
                            ) for l in logits_fake]) / len(logits_fake)
            loss_Dreal = sum([(F.relu(th.ones_like(l) - l)).mean(
                            ) for l in logits_real]) / len(logits_real)
            discriminator_loss = loss_Dreal + loss_Dgen
            if self.args.large_log:
                print("logits_real: ", sum([l.mean() for l in logits_real]).item(
                    ) / len(logits_real), len(logits_real))
                print("logits_fake: ", sum([l.mean() for l in logits_fake]).item(
                    ) / len(logits_fake), len(logits_fake))
    ...
    def ctm_losses(
        ...
        """
        Computes the CTM losses based on the given parameters.
        """
        ...
        if step % self.args.g_learning_period == 0 or not self.args.gan_training:
            ...
            x_t_dt = self.heun_solver(target_model, x_t, indices, dims,
                                      t, t_dt, ctm=ctm, num_step=num_heun_step,
                                      **model_kwargs).detach()
            ctm_target = self.get_ctm_target(x_t_dt, t_dt, s, model, target_model,
                                             ctm=ctm,
                                             inner_type=self.args.ctm_target_inner_type,
                                             **model_kwargs)
            ...
            snrs = self.get_snr(t)
            weights = get_weightings(self.args.weight_schedule,
                                     snrs, self.args.sigma_data,
                                     t, s, self.args.weight_schedule_multiplier)

            terms["consistency_loss"] = self.get_CTM_loss(ctm_estimate, ctm_target,
                                                          weights, step - init_step,)
            if self.args.large_log:
                if s != None:
                    print(f"{step}-th step, t, t-dt, s, weight, loss: ", t[0].item(
                          ), t_dt[0].item(), s[0].item(), weights[0].item(),
                          terms["consistency_loss"][0].item())
                else:
                    print(f"{step}-th step, t, t-dt, weight, loss: ",
                          t[0].item(), t_dt[0].item(), terms["consistency_loss"][0].item(
                          ), weights[0].item())
            if self.args.diffusion_training:
                if diffusion_training_:
                    terms['denoising_loss'] = self.get_DSM_loss(model,
                                              x_start, model_kwargs,
                                              terms["consistency_loss"],
                                              step, init_step)

            if self.args.gan_training and step - init_step >= self.args.discriminator_start_itr:
                if gan_training_:
                    gan_x_t, gan_t, gan_t_dt, gan_s, _, _ = self.get_gan_time(x_start, noise,
                                                                              x_t, t, t_dt,
                                                                              s, indices,
                                                                              num_heun_step,
                                                                              gan_num_heun_step)
                    gan_fake = self.get_gan_fake(ctm_estimate, gan_x_t, gan_t,
                                                 gan_t_dt, gan_s, model, target_model,
                                                 ctm,step - init_step,
                                                 **model_kwargs)
                    terms['d_loss'] = self.get_GAN_loss(model,
                                                    fake=gan_fake,
            ...
            gan_x_t, gan_t, gan_t_dt, gan_s, gan_indices, gan_num_heun_step = \
                self.get_gan_time(x_start, noise, x_t, t, t_dt,
                                  s, indices, num_heun_step,
                                  gan_num_heun_step)
            gan_real = self.get_gan_real(x_start, gan_x_t, gan_t, gan_t_dt,
                                         gan_s, gan_indices, dims,
                                         gan_num_heun_step,
                                         model, target_model,
                                         ctm, step - init_step, **model_kwargs)
            gan_fake = self.get_gan_fake(ctm_estimate, gan_x_t, gan_t, gan_t_dt,
                                         gan_s, model, target_model, ctm,
