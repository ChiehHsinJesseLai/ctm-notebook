...
class TrainLoop:
    """
    A class to manage the training loop for a model with optional 
    discriminator and diffusion components.
    """
    def __init__(
        self,
        #*,
        ...
        """
        Initialize the TrainLoop with the given parameters
        """
        self.args = args
        self.model = model
        if self.args.sanity_check:
            for name, param in self.model.named_parameters():
                logger.log(
                    "check and understand how consistency-type models override model parameters"
                    )
                logger.log("model parameter before overriding: ",
                           param.data.cpu().detach().reshape(-1)[:3])
                break
        ...
        self.x_T = self.generator.randn(*(self.args.sampling_batch,
        ...
        if self.args.class_cond:
            self.classes = self.generator.randint(0,
                                                self.args.num_classes, (
                                                self.args.sampling_batch,), device='cpu')
        ...
        if self.args.sanity_check:
            for name, param in self.model.named_parameters():
                logger.log("model parameter after overriding: ",
                           param.data.cpu().detach().reshape(-1)[:3])
                break
        if self.discriminator != None:
            if self.args.sanity_check:
                for name, param in self.discriminator.named_parameters():
                    logger.log("discriminator parameter before overriding: ",
                               param.data.cpu().detach().reshape(-1)[:3])
                    break
            self._load_and_sync_discriminator_parameters()
            if self.args.sanity_check:
                for name, param in self.discriminator.named_parameters():
                    logger.log("discriminator parameter after overriding: ",
                               param.data.cpu().detach().reshape(-1)[:3])
                    break
        ...
        if self.args.sanity_check:
            logger.log("mp trainer master parameter: ",
                    "(should be same as model parameter if no linear probing): ",
        ...
        if self.discriminator != None:
            ...
            self.d_opt = RAdam(
                self.d_mp_trainer.master_params, lr=args.d_lr,
                weight_decay=self.args.weight_decay, betas=(0.5, 0.9))
        if self.resume_step:
            self._load_optimizer_state()
            if self.discriminator != None:
                try:
                    if self.args.sanity_check:
                        print("discriminator opt state dict before overriding: ",
                              self.d_opt.state_dict())
                    self._load_d_optimizer_state()
                    if self.args.sanity_check:
                        print("discriminator opt state dict after overriding: ",
                              self.d_opt.state_dict())
                except:
    ...
    def _load_and_sync_parameters(self):
        """
        Load and synchronize model parameters.
        """
    ...
    def _load_and_sync_discriminator_parameters(self):
        """
        Load and synchronize discriminator parameters.
        """
        resume_checkpoint = find_resume_checkpoint() or self.args.resume_checkpoint
        if resume_checkpoint:
            self.resume_step = parse_resume_step_from_filename(resume_checkpoint)
            resume_checkpoint = bf.join(bf.dirname(resume_checkpoint),
                                        f"d_model{self.resume_step:06}.pt")
            if dist.get_rank() == 0:
                if os.path.exists(resume_checkpoint):
                    logger.log(f"loading discriminator model from checkpoint: ",
                               f"{resume_checkpoint}...")
                    #try:
                    if dist.get_world_size() > 1:
                    ...
                    logger.log(
                        f"end loading discriminator model from checkpoint: ",
                        f"{resume_checkpoint}...")
    ...
    def _load_ema_parameters(self, rate):
        """
        Load the exponential moving average (EMA) parameters.
        """
    ...
    def _update_ema(self):
        """
        Update the exponential moving average (EMA) parameters.
        """
    ...
    def _anneal_lr(self):
        """
        Anneal the learning rate based on the current step.
        """
    ...
    def log_step(self):
        """
        Log the current training step and number of samples.
        """
        logger.logkv("step", self.step + self.resume_step)
        logger.logkv("samples", (self.step + self.resume_step + 1) * self.global_batch)

    def sampling(self, model, sampler, ctm=None,
                 teacher=False, step=-1,
                 num_samples=-1, batch_size=-1,
                 rate=0.999,
                 png=False, resize=True,
                 generator=None,
                 class_generator=None,
                 sample_dir=''):
        """
        Perform sampling using the given model and sampler.
        """
        if not teacher:
            model.eval()
        if step == -1:
            ...
            with th.no_grad():
                ...
                if self.args.class_cond:
                    if self.args.train_classes >= 0:
                        classes = th.ones(size=(batch_size,),
                                          device=dist_util.dev(),
                                          dtype=int) * self.args.train_classes
                        model_kwargs["y"] = classes
                    elif self.args.train_classes == -2:
                        classes = [0, 1, 9, 11, 29, 31, 33, 55, 76,
                                   89, 90, 130, 207, 250, 279, 281,
                                   291, 323, 386, 387,388, 417, 562,
                                   614, 759, 789, 800, 812, 848, 933, 973, 980]
                        assert batch_size % len(classes) == 0
                        model_kwargs["y"] = th.tensor([x for x in classes for _ in range(
                            batch_size // len(classes))], device=dist_util.dev())
                    else:
                        if class_generator != None:
                            model_kwargs["y"] = class_generator.randint(0,
                                                                        self.args.num_classes,
                                                                        (batch_size,),
                                                                        device=dist_util.dev())
                        else:
                            if num_samples == -1:
                                ...
                                model_kwargs["y"] = th.randint(0, self.args.num_classes,
                                                               size=(batch_size, ),
                                                               device=dist_util.dev())
                if generator != None:
                    x_T = generator.randn(*(batch_size, self.args.in_channels,
                ...
                if dist.get_rank() == 0:
                    ...
                    image_grid = make_grid(th.tensor(arr).permute(0, 3, 1, 2) / 255.,
                                           nrow, padding=2)
                    if num_samples == -1:
                        with bf.BlobFile(bf.join(
                            get_blob_logdir(),
                            f"{'teacher_' if teacher else ''}sample_{sampler}_",
                            f"sampling_step_{step}_step_{self.step}.png"), "wb") as fout:
                            save_image(image_grid, fout)
                    else:
                        if generator != None:
                            os.makedirs(bf.join(get_blob_logdir(), sample_dir),
                                        exist_ok=True)
                            np.savez(bf.join(get_blob_logdir(),
                                             f"{sample_dir}/sample_{number // arr.shape[0]}.npz"),
                                     arr)
                            if png and number <= 3000:
                                with bf.BlobFile(bf.join(
                                    get_blob_logdir(),
                                    f"{sample_dir}/sample_{number // arr.shape[0]}.png"
                                    ), "wb"
                                    ) as fout:
                                    save_image(image_grid, fout)
                        else:
    ...
    def calculate_similarity_metrics(self, image_path,
                                     num_samples=50000,
                                     step=1, batch_size=100,
                                     rate=0.999, sampler='exact',
                                     log=True):
        """
        Calculate similarity metrics (PSNR and SSIM) between generated and reference images
        """
        ...
        if log:
            logger.log(
                f"{self.step}-th step {sampler} sampler (NFE {step}) EMA {rate} ",
                f"PSNR-{num_samples // 1000}k: {psnr}, ",
                f"SSIM-{num_samples // 1000}k: {ssim}")
        else:
            return psnr, ssim

    def calculate_inception_stats(self, data_name,
                                  image_path,
                                  num_samples=50000,
                                  batch_size=100,
                                  device=th.device('cuda')):
        """
        Calculate inception statistics (mean and covariance) for generated images.
        """
        if data_name.lower() == 'cifar10':
            ...
            sample_stats, sample_stats_spatial = tuple(
                self.evaluator.compute_statistics(x) for x in sample_acts)
    ...
    def compute_fid(self, mu, sigma, ref_mu=None, ref_sigma=None):
        """
        Compute the Frechet Inception Distance (FID) between generated and reference images.
        """
        ...
        return fid
    def calculate_inception_stats_npz(self, image_path,
                                      num_samples=50000,
                                      step=1, batch_size=100,
                                      device=th.device('cuda'),
                                      rate=0.999):
        """
        Calculate inception statistics (mean and covariance) for generated images from NPZ files.
        """
        print(f'Loading images from "{image_path}"...')
        mu = th.zeros([self.feature_dim], dtype=th.float64, device=device)
        sigma = th.zeros([self.feature_dim, self.feature_dim], dtype=th.float64, device=device)
        ...
        fid = float(np.real(fid))
        assert num_samples % 1000 == 0
        logger.log(f"{self.step}-th step: "
                   f"NFE {step}, "
                   f"EMA {rate}, "
                   f"FID-{num_samples // 1000}k: {fid}")
class CMTrainLoop(TrainLoop):
    """
    A class to manage the training loop for a consistency model
    with optional target and teacher models.
    """
    def __init__(
        self,
        *,
        ...
        **kwargs,
        ...
        """
        Initialize the CMTrainLoop with the given parameters.
        """
        ...
        if target_model:
            if self.args.sanity_check:
                for name, param in self.target_model.named_parameters():
                    logger.log("target model parameter before overriding: ",
                               param.data.cpu().detach().reshape(-1)[:3])
                    break
            self._load_and_sync_ema_parameters_to_target_parameters()
            if self.args.sanity_check:
                for name, param in self.target_model.named_parameters():
                    logger.log("target model parameter after overriding: ",
                               param.data.cpu().detach().reshape(-1)[:3])
                    break
            ...
            if self.args.use_fp16:
                ...
                self.target_model_param_groups_and_shapes = list(
            ...
            if self.args.sanity_check:
                for name, param in self.target_model.named_parameters():
                    logger.log("target model parameter after all: ",
                               param.data.cpu().detach().reshape(-1)[:3])
                    break
        ...
        if self.args.check_dm_performance:
            if not os.path.exists(self.args.dm_sample_path_seed_42):
                self.sampling(model=self.teacher_model, sampler='heun',
                              teacher=True,
                              step=18 if self.args.data_name.lower() == 'cifar10' else 40,
                              num_samples=self.args.eval_num_samples,
                              batch_size=self.args.eval_batch,
                              rate=0.0, ctm=False, png=False,
                              resize=False,
                              generator=get_generator('determ',
                                                      self.args.eval_num_samples,
                                                      self.args.eval_seed),
                              class_generator=get_generator('determ',
                                                            self.args.eval_num_samples,
                                                            0),
        ...
        if self.args.data_name.lower() == 'cifar10':
            if dist.get_rank() == 0:
                ...
                if self.args.check_dm_performance:
                    if self.args.ae_image_path_seed_42 != '':
                        self.ae_mu, self.ae_sigma = self.calculate_inception_stats(
                            self.args.data_name,
                            self.args.ae_image_path_seed_42,
                            num_samples=self.args.eval_num_samples)
                    self.dm_mu, self.dm_sigma = self.calculate_inception_stats(
                        self.args.data_name,
                        self.args.dm_sample_path_seed_42,
                        num_samples=self.args.eval_num_samples)
                    logger.log(f"DM FID-50k: {self.compute_fid(self.dm_mu,self.dm_sigma)}")
                    ref_files = glob.glob(os.path.join(self.args.dm_sample_path_seed_42,
                                                       'sample*.npz'))
                    ref_files.sort()
                    self.ref_images = []
                    ...
                    if self.args.ae_image_path_seed_42 != '':
                        logger.log(f"Regenerated DM Samples (by LSGM AE) "
                                   f"FID-50k: {self.compute_fid(self.ae_mu, self.ae_sigma)}")
            ...
            if dist.get_rank() == 0:
                ...
                self.ref_stats, self.ref_stats_spatial = self.evaluator.read_statistics(
                    self.args.ref_path,
                    self.ref_acts)
                if self.args.check_dm_performance:
                    if os.path.exists(os.path.join(os.path.join(self.args.dm_sample_path_seed_42,
                                                                'single_npz'), f'stats')):
                        with open(os.path.join(os.path.join(self.args.dm_sample_path_seed_42,
                                                            'single_npz'), f'acts'), 'rb') as f:
                            sample_acts = pickle.load(f)
                            sample_acts = (sample_acts['acts'], sample_acts['acts_spatial'])
                        with open(os.path.join(os.path.join(self.args.dm_sample_path_seed_42,
                                                            'single_npz'), f'stats'), 'rb') as f:
                            sample_stats = pickle.load(f)
                            sample_stats, sample_stats_spatial = (sample_stats['stats'],
                                                                  sample_stats['stats_spatial'])
                    else:
                        sample_acts, sample_stats, sample_stats_spatial = self.calculate_inception_stats(
                                                                    self.args.data_name,
                                                                    self.args.dm_sample_path_seed_42,
                                                                    num_samples=self.args.eval_num_samples)
                    logger.log("Inception Score-50k:",
                               self.evaluator.compute_inception_score(sample_acts[0]))
                    logger.log("FID-50k:",
                               sample_stats.frechet_distance(self.ref_stats))
                    logger.log("sFID-50k:",
                               sample_stats_spatial.frechet_distance(self.ref_stats_spatial))
                    prec, recall = self.evaluator.compute_prec_recall(self.ref_acts[0],
                                                                      sample_acts[0])
    ...
    def print_gpu_usage(self, prefix=''):
        """
        Print the GPU usage with a given prefix.
        """
    ...
    def _load_and_sync_target_parameters(self):
        """
        Load and synchronize target model parameters from a checkpoint.
        """
        resume_checkpoint = find_resume_checkpoint() or self.args.resume_checkpoint
        if resume_checkpoint:
    ...
    def _load_and_sync_teacher_parameters(self):
        """
        Load and synchronize teacher model parameters from a checkpoint.
        """
        resume_checkpoint = find_resume_checkpoint() or self.args.resume_checkpoint
        if resume_checkpoint:
            path, name = os.path.split(resume_checkpoint)
    ...
    def run_loop(self):
        ...
        while (
            ...
            if self.args.intermediate_samples:
                if dist.get_rank() == 0:
                    if (self.step == self.initial_step + 10 or
                        self.step % self.args.sample_interval == self.args.sample_interval - 1):
                        if self.args.training_mode.lower() == 'ctm':
                            if self.args.consistency_weight > 0.:
                                ...
                                self.sampling(model=self.ddp_model, sampler='heun',
                                              ctm=True, teacher=True)
                    ...
                    if self.step == self.initial_step + 10 and self.teacher_model != None:
                        self.sampling(model=self.teacher_model, sampler='heun',
                                      ctm=False, teacher=True)
            self.run_step(batch, cond)
            if self.args.gpu_usage:
                self.print_gpu_usage('After one step training')
            if self.args.large_log:
                print("mp trainer master parameter after one step update: ",
                      self.mp_trainer.master_params[1].reshape(-1)[:3])
                for name, param in self.model.named_parameters():
                    print("model parameter after one step update: ",
                          param.data.cpu().detach().reshape(-1)[:3])
                    break
                for name, param in self.target_model.named_parameters():
                    print("target model parameter after one step update: ",
                          param.data.cpu().detach().reshape(-1)[:3])
                    break
            if self.args.check_ctm_denoising_ability:
                self.eval(step=18, sampler='heun', teacher=True,
                          ctm=True, rate=0.0)
            if (
                self.global_step
                ...
                if self.args.linear_probing:
                    self.eval(step=18, sampler='heun',
                              teacher=True, ctm=True,
                              rate=0.0,
                              generator=get_generator('determ',
                                                      self.args.eval_num_samples,
                                                      self.args.eval_seed),
                ...
                for rate, params in zip(self.ema_rate, self.ema_params):
                    if rate == 0.999:
                        ...
                        if dist.get_rank() == 0:
                            ...
                            logger.log("FID by iteration (NFE 1, EMA 0.999): ",
                                       self.fids)
            ...
            if self.args.large_log:
                print("mp trainer master parameter after sampling: ",
                      self.mp_trainer.master_params[1].reshape(-1)[:3])
                for name, param in self.model.named_parameters():
                    print("model parameter after sampling: ",
                          param.data.cpu().detach().reshape(-1)[:3])
                    break
                for name, param in self.target_model.named_parameters():
                    print("target model parameter after sampling: ",
                          param.data.cpu().detach().reshape(-1)[:3])
                    break
            ...
            if self.args.large_log:
                print("mp trainer master parameter after saving: ",
                      self.mp_trainer.master_params[1].reshape(-1)[:3])
                for name, param in self.model.named_parameters():
                    print("model parameter after saving: ",
                          param.data.cpu().detach().reshape(-1)[:3])
                    break
                for name, param in self.target_model.named_parameters():
                    print("target model parameter after saving: ",
                          param.data.cpu().detach().reshape(-1)[:3])
                    break
                print(f"0.999 ema param after overriding: ",
    ...
    def save_check(self, rate):
        """
        Perform a save check and evaluate the model.
        """
        if self.args.training_mode.lower() == 'ctm':
            assert rate == 0.999
            fid = self.eval(step=1,
                            rate=rate,
                            ctm=True,
                            generator=get_generator(
                                'determ',self.args.eval_num_samples,
                                self.args.eval_seed),
                                class_generator=get_generator(
                                    'determ',self.args.eval_num_samples, 0),
    ...
    def evaluation(self, rate):
        """
        Evaluate the model based on the given rate.
        """
        if self.args.training_mode.lower() == 'ctm':
            ...
            if self.args.eval_similarity:
                self.eval(step=1,
                          ...
                          ctm=True,
                          generator=get_generator(
                              'determ',self.args.eval_num_samples,self.args.eval_seed),
                              class_generator=get_generator('determ',
                                                            self.args.eval_num_samples,0),
                                                            metric='similarity',
                                                            delete=True)
            if self.args.eval_fid:
            ...
            if self.args.large_nfe_eval:
                ...
                if self.args.eval_similarity:
                    self.eval(step=step, rate=rate, ctm=True,
                              generator=get_generator('determ',
                                                      self.args.eval_num_samples,
                                                      self.args.eval_seed),
                                                      class_generator=get_generator(
    ...
    def run_step(self, batch, cond):
        """
        Run a single training step with the given batch and conditions.
        """
        if self.args.large_log:
            print("mp trainer master parameter before update: ",
                  self.mp_trainer.master_params[1].reshape(-1)[:3])
            for name, param in self.model.named_parameters():
                print("model parameter before update: ",
                      param.data.cpu().detach().reshape(-1)[:3])
                break
            for name, param in self.target_model.named_parameters():
                print("target model parameter before update: ",
                      param.data.cpu().detach().reshape(-1)[:3])
                break
    ...
    def _update_target_ema(self):
        """
        Update the exponential moving average (EMA) parameters for the target model
        """
    ...
    def forward_backward(self, batch, cond):
        """
        Perform the forward and backward passes for the given batch and conditions.
        """
        self.mp_trainer.zero_grad()
        if self.discriminator != None:
            self.d_mp_trainer.zero_grad()
        num_heun_step = [self.diffusion.get_num_heun_step(
            num_heun_step=self.args.num_heun_step)]
        if self.args.gan_specific_time:
            gan_num_heun_step = [self.diffusion.get_num_heun_step(
                num_heun_step=self.args.gan_num_heun_step,
        ...
        dist.broadcast_object_list(num_heun_step, src=0,device=dist_util.dev())
        if self.args.gan_specific_time:
            dist.broadcast_object_list(gan_num_heun_step, src=0,device=dist_util.dev())
        dist.broadcast_object_list(diffusion_training_, src=0,device=dist_util.dev())
        dist.broadcast_object_list(gan_training_, src=0,device=dist_util.dev())
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            if 'consistency_loss' in list(losses.keys()):
                ...
                if 'd_loss' in list(losses.keys()):
                    if self.args.large_log:
                        print("GAN learning, ",
                              self.args.discriminator_weight,
                              losses['d_loss'].mean())
                ...
                if self.args.sanity_check:
                    ...
                    for name, param in self.model.named_parameters():
                        print(
                            "model parameter gradient for current microbatch: ",
                            th.autograd.grad(
                                outputs=(2 ** self.mp_trainer.lg_loss_scale) * loss,
                                inputs=param,
                                retain_graph=True)[0].reshape(-1)[:3])
            ...
            if self.args.sanity_check:
                print("rank: ", dist.get_rank())
                for name, param in self.model.named_parameters():
                    print("model parameter gradient across all microbatch: ",
                          param.grad.cpu().detach().reshape(-1)[:3])
                    break

    @th.no_grad()
    def eval(self, step=1, sampler='exact', teacher=False,
             ctm=False, rate=0.999, generator=None,
             class_generator=None, metric='fid',
             delete=False, out=False):
        """
        Evaluate the model using the specified parameters.
        """
        model = self.model
        ...
        if dist.get_rank() == 0:
            if self.args.data_name.lower() == 'cifar10':
                if metric == 'fid':
                    ...
                    logger.log(
                        f"{self.step}-th step {sampler} sampler (NFE {step}) EMA {rate} "
                        f"FID-{self.args.eval_num_samples // 1000}k: "
                        f"{self.compute_fid(mu, sigma)}")
                if metric == 'similarity':
                    mu, sigma = self.calculate_inception_stats(
                        ...
                        num_samples=self.args.eval_num_samples)
                    logger.log(
                        f"{self.step}-th step {sampler} sampler (NFE {step}) seed 42 EMA {rate} "
                        f"FID-{self.args.eval_num_samples // 1000}k: "
                        f"{self.compute_fid(mu, sigma)}")
                ...
                if self.args.check_dm_performance:
                    logger.log(
                        f"{self.step}-th step {sampler} sampler (NFE {step}) EMA {rate} "
                        f"FID-{self.args.eval_num_samples // 1000}k compared with DM: "
                        f"{self.compute_fid(mu, sigma, self.dm_mu, self.dm_sigma)}")
                ...
                sample_acts, sample_stats, sample_stats_spatial = self.calculate_inception_stats(
                    ...
                    num_samples=self.args.eval_num_samples)
                logger.log(f"Inception Score-{self.args.eval_num_samples // 1000}k:",
                           self.evaluator.compute_inception_score(sample_acts[0]))
                logger.log(f"FID-{self.args.eval_num_samples // 1000}k:",
                           sample_stats.frechet_distance(self.ref_stats))
                logger.log(f"sFID-{self.args.eval_num_samples // 1000}k:",
                           sample_stats_spatial.frechet_distance(self.ref_stats_spatial))
                prec, recall = self.evaluator.compute_prec_recall(self.ref_acts[0],
                                                                  sample_acts[0])
                logger.log("Precision:", prec)
    ...
    def d_save(self):
        """
        Save the discriminator optimizer and model state.
        """
        ...
        if dist.get_rank() == 0:
            ...
            with bf.BlobFile(
                bf.join(get_blob_logdir(),
                        f"d_model{self.global_step:06d}.pt"), "wb") as f:
                th.save(self.d_mp_trainer.master_params_to_state_dict(
                    self.d_mp_trainer.master_params), f)
...
def find_ema_checkpoint(main_checkpoint, step, rate):
    """
    Find the EMA checkpoint corresponding to the given main checkpoint, step, and rate.
    """
...
def log_loss_dict(losses):
    """
    Log the loss dictionary with mean and standard deviation.
    """
