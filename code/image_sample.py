...
def main():
    """
    Main function to set up and run the model and diffusion process.
    ...
    if args.sampler in ['multistep', 'exact', 'cm_multistep']:
        out_dir = os.path.join(args.out_dir,
                    f'{args.training_mode}_{args.sampler}_sampler_'
                    f'{args.sampling_steps}_steps_{step}_itrs_{ema}_ema_'
                    f'{"".join([str(i) for i in ts])}')
    elif args.sampler in ["gamma"]:
        out_dir = os.path.join(args.out_dir,
                  f'{args.training_mode}_{args.sampler}_sampler_'
                  f'{args.sampling_steps}_steps_{step}_itrs_{ema}_ema_'
                  f'{"".join([str(i) for i in ts])}_ind1_{args.ind_1}_ind2_{args.ind_2}')
    elif args.sampler in ["gamma_multistep"]:
        out_dir = os.path.join(args.out_dir,
                               f'{args.training_mode}_{args.sampler}_sampler_'
                               f'{args.sampling_steps}_steps_{step}_itrs_{ema}_ema_'
                               f'{"".join([str(i) for i in ts])}_gamma_{args.gamma}')
    else:
        out_dir = os.path.join(args.out_dir,
                               f'{args.training_mode}_{args.sampler}_sampler_'
                               f'{args.sampling_steps}_steps_{step}_itrs_{ema}_ema')
    ...
    while itr * args.batch_size < args.eval_num_samples:
        ...
        if args.class_cond:
            if args.train_classes >= 0:
                classes = th.ones(size=(args.batch_size,),
                                   device=dist_util.dev(), dtype=int
                                   ) * int(args.train_classes)
                ...
                classes = th.tensor([x for x in classes for _ in range(
                    args.batch_size // len(classes))], device=dist_util.dev())
            else:
                classes = th.randint(low=0, high=args.num_classes,
        ...
        with th.no_grad():
            x = karras_sample(
                ...
                clip_denoised=False if args.data_name in ['church'] else True
                  if args.training_mode=='edm' else args.clip_denoised,
            ...
            if args.save_format == 'npz':
                if args.class_cond:
                    if args.classifier_guidance:
                        ...
                        np.savez(os.path.join(out_dir, f"sample_{r}.npz"),
...
def create_argparser():
    """
    Create an argument parser with default values for various parameters.
    """
