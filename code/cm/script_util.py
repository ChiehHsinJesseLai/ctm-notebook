...
def ctm_data_defaults(data_name):
    """
    Performs default configuration for CTM data and returns a dictionary.
    """
    return dict(
        train_classes=-1,
        type='png',
        sigma_data=0.5,
        deterministic=False,
        num_classes=10 if data_name == "cifar10" else 0 if data_name == "lsun_bedroom" else 1000,
    )

def ctm_loss_defaults(data_name):
    """
    Performs default configuration for CTM loss and returns a dictionary.
    """
    return dict(
        # CTM hyperparams
...
def ctm_train_defaults(data_name):
    """
    Performs default configuration for CTM training and returns a dictionary
    """
    return dict(
        beta_min=0.1,
        beta_max=20.,
...
def ctm_eval_defaults(data_name):
    """
    Performs default configuration for CTM evaluation and returns a dictionary.
    ...
    return dict(
        ...
        ref_path='/home/acf15618av/EighthArticleExperimentalResults/CIFAR10/author_ckpt/cifar10-32x32.npz'
                ...
                else "/home/pranay_sard/CTM_project/CTM/author_ckpt/VIRTUAL_lsun_bedroom256.npz" \
                if data_name == "lsun_bedroom" \
...
def cm_train_defaults(data_name):
    """
    Performs default configuration for CM training and returns a dictionary.

    """
    return dict(
        teacher_model_path="/home/acf15618av/EighthArticleExperimentalResults/CIFAR10/author_ckpt/edm-cifar10-32x32-uncond-vp.pkl"
        if data_name == 'cifar10' \
        else "/home/pranay_sard/CTM_project/CTM/author_ckpt/cd_bedroom256_lpips.pt" \
        if data_name == "lsun_bedroom" else "",
        teacher_dropout=0.0 if data_name == 'cifar10' else 0.1,
        training_mode="ctm",
        target_ema_mode="fixed",
        scale_mode="fixed",
        total_training_steps=23000,
        start_ema=0.999,
...
def model_and_diffusion_defaults(data_name):
    ...
    res = dict(
        ...
        sigma_max=80.0,
        rho=7,
        image_size=32 if data_name == 'cifar10' else 256 if data_name == "lsun_bedroom" else 64,
        num_channels=192,
        num_res_blocks=3,
        num_heads=4,
        ...
        channel_mult="",
        dropout=0.0,
        class_cond=False if data_name == 'cifar10' or data_name == "lsun_bedroom" else True,
...
def train_defaults(data_name):
    ...
    res = dict(
        out_dir="",
        data_dir="/home/acf15618av/dataset/CIFAR10/train" if data_name == 'cifar10' \
            else "/data2/LSUN_Bedroom/train_orig_imgs" if data_name == "lsun_bedroom" else "",
        schedule_sampler="uniform",
        lr=0.0004 if data_name == 'cifar10' else 0.00001 if data_name == "lsun_bedroom" else 0.000008,
        weight_decay=0.0,
        lr_anneal_steps=0,
        global_batch_size=128 if data_name == 'cifar10' else 512 if data_name == "lsun_bedroom" else 2048,
        batch_size=-1,
        microbatch=64 if data_name.lower() == 'cifar10' else 16 if data_name == "lsun_bedroom" else -1,  # -1 disables microbatches
        ema_rate="0.999,0.9999" if data_name == 'cifar10' or data_name == "lsun_bedroom" else "0.999,0.9999,0.9999432189950708",
...
def save(x, save_dir, name, npz=False):
    """
    Performs saving of images and optionally saves them as .npz files.
    """
...
def create_classifier(
    ...
    """
     Performs creation of a classifier model based on the given 
     parameters and returns the model.
    """
    if image_size == 512:
...
def create_model_and_diffusion(args, feature_extractor=None,
                               discriminator_feature_extractor=None,
                               teacher=False):
    """
    Performs creation of a model and diffusion process based on the
    given arguments and returns them.
    """
    schedule_sampler = create_named_schedule_sampler(args, args.schedule_sampler,
                                                     args.start_scales)
    diffusion_schedule_sampler = create_named_schedule_sampler(args,
                                                               args.diffusion_schedule_sampler,
                                                               args.start_scales)

    if args.data_name.lower() == 'cifar10':
        from cm.networks import EDMPrecond_CTM
        model = EDMPrecond_CTM(img_resolution=args.image_size,
                               img_channels=3,
...
def create_model(
    ...
    """
    Performs creation of a UNet model based on the given parameters and returns the model.
    """
    if channel_mult == "":
        if image_size == 512:
            channel_mult = (0.5, 1, 1, 2, 2, 4, 4)
...
def create_ema_and_scales_fn(
    ...
    """
     Performs creation of a function to compute EMA and scales 
     based on the given parameters and returns the function.
    """
    def ema_and_scales_fn(step):
        """
        computes the Exponential Moving Average (EMA) and scales 
        """
...
def add_dict_to_argparser(parser, default_dict):
    """
    Performs addition of dictionary items to an argument parser.
    """
...
def args_to_dict(args, keys):
    """
    Performs conversion of arguments to a dictionary based on specified keys
