...
def weight_init(shape, mode, fan_in, fan_out):
    ...
    """
    if mode == 'xavier_uniform': return np.sqrt(6 / (
                fan_in + fan_out)) * (torch.rand(*shape) * 2 - 1)
    if mode == 'xavier_normal':  return np.sqrt(2 / (
                fan_in + fan_out)) * torch.randn(*shape)
    if mode == 'kaiming_uniform': return np.sqrt(
                3 / fan_in) * (torch.rand(*shape) * 2 - 1)
    if mode == 'kaiming_normal':  return np.sqrt(
...
class Linear(torch.nn.Module):
    """
    Fully-connected layer with optional weight and bias initialization.
    """
    def __init__(self, in_features, out_features,
                 bias=True, init_mode='kaiming_normal',
...
class Conv2d(torch.nn.Module):
    """
    Convolutional layer with optional up/downsampling and weight/bias initialization.
    """
    def __init__(self,
        ...
        self.weight = torch.nn.Parameter(weight_init([out_channels, in_channels,
                                        kernel, kernel], **init_kwargs
                                        ) * init_weight) if kernel else None
        self.bias = torch.nn.Parameter(weight_init([out_channels], **init_kwargs
    ...
    def forward(self, x):
        ...
        if self.fused_resample and self.up and w is not None:
            x = torch.nn.functional.conv_transpose2d(x,
                                ...
                                ]), groups=self.in_channels, stride=2, padding=max(
                                f_pad - w_pad, 0))
            x = torch.nn.functional.conv2d(x, w, padding=max(w_pad - f_pad, 0))
        elif self.fused_resample and self.down and w is not None:
            x = torch.nn.functional.conv2d(x, w, padding=w_pad+f_pad)
            x = torch.nn.functional.conv2d(x, f.tile([self.out_channels, 1, 1, 1
            ...
            if self.down:
                x = torch.nn.functional.conv2d(x, f.tile([self.in_channels, 1, 1, 1]),
                                               groups=self.in_channels, stride=2, padding=f_pad)
            if w is not None:
...
class AttentionOp(torch.autograd.Function):
    """
    Custom autograd function for attention weight computation.
    """
    @staticmethod
    def forward(ctx, q, k):
        """
...
class UNetBlock(torch.nn.Module):
    """
    UNet block with optional up/downsampling, attention, and adaptive scaling.
    ...
    def __init__(self,
        ...
        self.affine = Linear(in_features=emb_channels, out_features=out_channels*(
                            2 if adaptive_scale else 1), **init)
        if self.training_mode == 'ctm':
            self.affine_s = Linear(in_features=emb_channels,
        ...
        if out_channels != in_channels or up or down:
            ...
            self.skip = Conv2d(in_channels=in_channels,
        ...
        if linear_probing:
            ...
            self.conv0_train = Conv2d(in_channels=in_channels, out_channels=out_channels,
                                      kernel=3, up=up, down=down,
                                      resample_filter=resample_filter, **init)
            self.norm1_train = GroupNorm(num_channels=out_channels, eps=eps)
            self.conv1_train = Conv2d(in_channels=out_channels, out_channels=out_channels,
                                      kernel=3, **init_zero)

            self.skip_train = None
            if out_channels != in_channels or up or down:
                kernel = 1 if resample_proj or out_channels != in_channels else 0
                self.skip_train = Conv2d(in_channels=in_channels, out_channels=out_channels,
                                         kernel=kernel, up=up, down=down,
                                   resample_filter=resample_filter, **init)

            if self.num_heads:
                self.norm2_train = GroupNorm(num_channels=out_channels, eps=eps)
                self.qkv_train = Conv2d(in_channels=out_channels,
                                        out_channels=out_channels * 3, kernel=1,
                                  **(init_attn if init_attn is not None else init))
                self.proj_train = Conv2d(in_channels=out_channels, out_channels=out_channels,
                                         kernel=1, **init_zero)
    ...
    def forward(self, x, emb, emb_s=None, emb_t=None):
        ...
        if self.linear_probing:
            ...
            y = self.conv1_train(torch.nn.functional.dropout(
                y, p=self.dropout, training=self.training))
            y = y.add_(self.skip_train(orig) if self.skip_train is not None else orig)
            y = y * self.skip_scale

            if self.num_heads:
                q, k, v = self.qkv_train(self.norm2_train(y
                          )).reshape(y.shape[0] * self.num_heads,
                        y.shape[1] // self.num_heads, 3,-1).unbind(2)
                w = AttentionOp.apply(q, k)
                a = torch.einsum('nqk,nck->ncq', w, v)
...
class FourierEmbedding(torch.nn.Module):
    ...
    def forward(self, x):
        """
        Forward pass for Fourier embedding.
        """
...
class SongUNet(torch.nn.Module):
    """
    UNet architecture for score-based generative modeling.
    """
    def __init__(self,
        ...
        augment_dim         = 0,            # Augmentation label dimensionality,
                                            # 0 = no augmentation.

        model_channels      = 128,          # Base multiplier for the number of channels.
        channel_mult        = [2,2,2],    # Per-resolution multipliers for the number of channels.
        channel_mult_emb    = 4,            # Multiplier for the dimensionality of
                                            # the embedding vector.
        num_blocks          = 4,            # Number of residual blocks per resolution.
        attn_resolutions    = [16],         # List of resolutions with self-attention.
        dropout             = 0.13,         # Dropout probability of intermediate activations.
        label_dropout       = 0,            # Dropout probability of class labels
                                            # for classifier-free guidance.

        embedding_type      = 'fourier',    # Timestep embedding type: 'positional' for DDPM++,
                                            # 'fourier' for NCSN++.
        channel_mult_noise  = 2,            # Timestep embedding size: 1 for DDPM++,
                                            # 2 for NCSN++.
        encoder_type        = 'residual',   # Encoder architecture: 'standard' for DDPM++,
                                            # 'residual' for NCSN++.
        decoder_type        = 'standard',   # Decoder architecture: 'standard'
                                            # for both DDPM++ and NCSN++.
        resample_filter     = [1,3,3,1],    # Resampling filter: [1,1] for DDPM++,
                                            # [1,3,3,1] for NCSN++.
        training_mode = '',
        ...
        block_kwargs = dict(
            emb_channels=emb_channels, num_heads=1,
            dropout=dropout, skip_scale=np.sqrt(0.5),
            eps=1e-6,resample_filter=resample_filter,
        ...
        self.map_noise = PositionalEmbedding(num_channels=noise_channels,
                                             endpoint=True
                            ) if embedding_type == 'positional' else FourierEmbedding(
        ...
        if self.training_mode.lower() == 'ctm':
            self.map_layer0_s = Linear(in_features=noise_channels,
                                       out_features=emb_channels, **init)
            self.map_layer1_s = Linear(in_features=emb_channels,
                                       out_features=emb_channels, **init)
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                cin = cout
                cout = model_channels
                self.enc[f'{res}x{res}_conv'] = Conv2d(in_channels=cin,
                                                       out_channels=cout,
                                                       kernel=3, **init)
            else:
                self.enc[f'{res}x{res}_down'] = UNetBlock(in_channels=cout,
                                                          out_channels=cout,
                                                          down=True,
                ...
                if encoder_type == 'skip':
                    self.enc[f'{res}x{res}_aux_down'] = Conv2d(in_channels=caux,
                                                               out_channels=caux,
                                                               kernel=0, down=True,
                                                               resample_filter=resample_filter)
                    self.enc[f'{res}x{res}_aux_skip'] = Conv2d(in_channels=caux,
                                                               out_channels=cout,
                                                               kernel=1, **init)
                if encoder_type == 'residual':
                    self.enc[f'{res}x{res}_aux_residual'] = Conv2d(in_channels=caux,
                                                                   out_channels=cout,
                                                                   kernel=3, down=True,
                                                                   resample_filter=resample_filter,
            ...
            for idx in range(num_blocks):
                ...
                self.enc[f'{res}x{res}_block{idx}'] = UNetBlock(in_channels=cin,
                                                                out_channels=cout,
                                                                attention=attn,
                                                                training_mode=training_mode,
                                                                linear_probing=linear_probing,
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            if level == len(channel_mult) - 1:
                self.dec[f'{res}x{res}_in0'] = UNetBlock(in_channels=cout,
                                                         out_channels=cout,
                                                         attention=True,
                                                         training_mode=training_mode,
                                                         linear_probing=linear_probing,
                ...
                self.dec[f'{res}x{res}_up'] = UNetBlock(in_channels=cout,
                                                        out_channels=cout,
                                                        up=True,
                                                        training_mode=training_mode,
                                                        linear_probing=linear_probing,
            ...
            for idx in range(num_blocks + 1):
                ...
                self.dec[f'{res}x{res}_block{idx}'] = UNetBlock(in_channels=cin,
                                                                out_channels=cout,
                                                                attention=attn,
                                                                training_mode=training_mode,
                                                                linear_probing=linear_probing,
            ...
            if decoder_type == 'skip' or level == 0:
                if decoder_type == 'skip' and level < len(channel_mult) - 1:
                    self.dec[f'{res}x{res}_aux_up'] = Conv2d(in_channels=out_channels,
                                                             out_channels=out_channels,
                                                             kernel=0, up=True,
                                                             resample_filter=resample_filter)
                self.dec[f'{res}x{res}_aux_norm'] = GroupNorm(num_channels=cout, eps=1e-6)
                self.dec[f'{res}x{res}_aux_conv'] = Conv2d(in_channels=cout,
                                                           out_channels=out_channels,
                                                           kernel=3, **init_zero)
                if self.linear_probing:
                    self.dec[f'{res}x{res}_aux_norm_train'] = GroupNorm(num_channels=cout,
                                                                        eps=1e-6)
                    self.dec[f'{res}x{res}_aux_lin_train'] = Linear(in_features=emb_channels,
                                            out_features=out_channels*img_resolution*img_resolution,
                                            **init)
                    self.dec[f'{res}x{res}_aux_conv_train'] = Conv2d(in_channels=cout,
    ...
    def forward(self, x, noise_labels, noise_labels_s, class_labels):
        ...
        if self.map_label is not None:
            ...
            if self.training and self.label_dropout:
                tmp = tmp * (torch.rand([x.shape[0], 1],
                            device=x.device) >= self.label_dropout).to(tmp.dtype)
        ...
        if noise_labels_s != None:
            ...
            emb_s = emb_s.reshape(emb_s.shape[0], 2, -1).flip(
                1).reshape(*emb_s.shape)  # swap sin/cos
            if self.map_label is not None:
                tmp = class_labels
                if self.training and self.label_dropout:
                    tmp = tmp * (torch.rand([x.shape[0], 1],
            ...
            if self.linear_probing:
                ...
                emb_t = emb_t.reshape(emb_t.shape[0],
                        2, -1).flip(1).reshape(*emb_t.shape)  # swap sin/cos
        ...
        for name, block in self.enc.items():
            if 'aux_down' in name:
                ...
                x = block(x, emb, emb_s=None if noise_labels_s == None else emb_s,
                          ...
                          ) if isinstance(block, UNetBlock) else block(x)
                skips.append(x)
        ...
        for name, block in self.dec.items():
            if 'aux_up' in name:
                ...
                emb_mult = (block(emb_t) - block(emb_s)).reshape(-1, 3,
                                                                self.img_resolution,
                                                                self.img_resolution)
...
class DhariwalUNet(torch.nn.Module):
    ...
    def __init__(self,
        ...
        model_channels      = 192,          # Base multiplier for the
                                            # number of channels.
        ...
        label_dropout       = 0,            # Dropout probability of class labels for
        ...
        self.map_label = Linear(in_features=label_dim, out_features=emb_channels,
                                bias=False, init_mode='kaiming_normal',
                                init_weight=np.sqrt(label_dim)) if label_dim else None
        if self.training_mode.lower() == 'ctm':
            self.map_layer0_s = Linear(in_features=model_channels,
                                       out_features=emb_channels, **init)
            self.map_layer1_s = Linear(in_features=emb_channels,
                                       out_features=emb_channels, **init)
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            if level == len(channel_mult) - 1:
                self.dec[f'{res}x{res}_in0'] = UNetBlock(in_channels=cout,
                                                         ...
                                                         training_mode=training_mode,
                                                         **block_kwargs)
                self.dec[f'{res}x{res}_in1'] = UNetBlock(in_channels=cout,
                                                         out_channels=cout,
                                                         training_mode=training_mode,
                                                         **block_kwargs)
            else:
        ...
        self.out_conv = Conv2d(in_channels=cout,
                               out_channels=out_channels,
                               kernel=3, **init_zero)

    def forward(self, x, noise_labels, noise_labels_s,
                class_labels, augment_labels=None):
        """
         Forward pass through the DhariwalUNet model.
        """
        # Mapping.
        emb = self.map_noise(noise_labels)
        ...
        for block in self.enc.values():
            x = block(x, emb,
                    emb_s=None if noise_labels_s == None else emb_s
                    ) if isinstance(block, UNetBlock) else block(x)
            skips.append(x)
...
class VPPrecond(torch.nn.Module):
    """
    Variance Preserving (VP) preconditioning for score-based generative modeling.
    ...
    def forward(self, x, sigma, class_labels=None, force_fp32=False, **model_kwargs):
        """
        Forward pass through the VP preconditioning model.
        ...
        class_labels = None if self.label_dim == 0 else torch.zeros([
                                1,self.label_dim],
                                device=x.device
                                ) if class_labels is None else class_labels.to(torch.float32
    ...
    def sigma_inv(self, sigma):
        """
        Compute the inverse of the noise level.
        """
        sigma = torch.as_tensor(sigma)
        return ((self.beta_min ** 2 + 2 * self.beta_d * (
            1 + sigma ** 2).log()).sqrt() - self.beta_min) / self.beta_d

    def round_sigma(self, sigma):
        """
        Round the noise level to the nearest value.
        """
...
class VEPrecond(torch.nn.Module):
    """
    Variance Exploding (VE) preconditioning for score-based generative modeling.
    """
    def __init__(self,
        img_resolution,                 # Image resolution.
        img_channels,                   # Number of color channels.
    ...
    def forward(self, x, sigma,
                ...
                **model_kwargs):
        """
        Forward pass through the VE preconditioning model.
        """
        x = x.to(torch.float32)
        sigma = sigma.to(torch.float32).reshape(-1, 1, 1, 1)
        class_labels = None if self.label_dim == 0 else torch.zeros([1,
                                                self.label_dim],
                                                device=x.device
                                                ) if class_labels is None else class_labels.to(
...
class iDDPMPrecond(torch.nn.Module):
    """
    Improved Denoising Diffusion Probabilistic Models (iDDPM) preconditioning.
    """
    def __init__(self,
        img_resolution,                     # Image resolution.
        img_channels,                       # Number of color channels.
        ...
        **model_kwargs,                     # Keyword arguments for the underlying model.
        ...
        for j in range(M, 0, -1): # M, ..., 1
            u[j - 1] = ((u[j] ** 2 + 1) / (self.alpha_bar(
            j - 1) / self.alpha_bar(j)).clip(min=C_1) - 1).sqrt()
    ...
    def forward(self, x, sigma, class_labels=None,
        ...
        """
        Forward pass through the iDDPM preconditioning model.
        """
        x = x.to(torch.float32)
        sigma = sigma.to(torch.float32).reshape(-1, 1, 1, 1)
        class_labels = None if self.label_dim == 0 else torch.zeros([1,
        ...
        c_noise = self.M - 1 - self.round_sigma(sigma,
                            return_index=True).to(torch.float32)
    ...
    def alpha_bar(self, j):
        """
        Compute the cumulative product of alpha values at timestep j.
        """
        j = torch.as_tensor(j)
    ...
    def round_sigma(self, sigma, return_index=False):
        ...
        """
        sigma = torch.as_tensor(sigma)
        index = torch.cdist(sigma.to(self.u.device).to(
                torch.float32).reshape(1, -1, 1), self.u.reshape(
                1, -1, 1)).argmin(2)
...
class EDMPrecond(torch.nn.Module):
    """
    Preconditioning for the Elucidating the Design Space of Diffusion-Based Generative Models (EDM).
    """
    def __init__(self,
        img_resolution,                     # Image resolution.
        img_channels,                       # Number of color channels.
    ...
    def forward(self, x, sigma, class_labels=None,
                force_fp32=False, **model_kwargs):
        """
        Forward pass through the EDM preconditioning model.
        """
        x = x.to(torch.float32)
        sigma = sigma.to(torch.float32).reshape(-1, 1, 1, 1)
        class_labels = None if self.label_dim == 0 else torch.zeros([1,
                                                ...
                                                torch.float32).reshape(-1, self.label_dim)
        dtype = torch.float16 if (self.use_fp16 and not force_fp32 and x.device.type == 'cuda'
                                  ) else torch.float32
...
class EDMPrecond_CTM(torch.nn.Module):
    """
    Improved preconditioning for the Elucidating the Design 
    Space of Diffusion-Based Generative Models (EDM).
    """
    def __init__(self,
        img_resolution,                     # Image resolution.
        ...
        **model_kwargs,                     # Keyword arguments for the underlying model.
        ...
        if teacher:
            ...
            if teacher_model_path.split(".")[-1] == "pkl":
    ...
    def get_c_in(self, sigma):
        """
        Compute the input scaling factor.
        """
        return 1 / (sigma**2 + self.sigma_data**2) ** 0.5

    def unrescaling_t(self, rescaled_t):
        """
        Convert rescaled time to original time.
        """
        return torch.exp(rescaled_t / 250.) - 1e-44

    def forward(self, rescaled_x, rescaled_t,
                s=None, teacher=False,
                **model_kwargs):
        """
        Forward pass through the EDM preconditioning model with CTM.
        """
        class_labels = None if self.label_dim == 0 else torch.zeros([1, self.label_dim],
                                                 device=rescaled_x.device) \
        ...
        if self.teacher:
            ...
            c_skip = append_dims(self.sigma_data ** 2 / (sigma ** 2 + self.sigma_data ** 2),
                                 rescaled_x.ndim)
            c_out = append_dims(sigma * self.sigma_data / (
            ...
            if s != None:
                ...
                s = s.log() / 4
            F_x = self.model(rescaled_x.to(dtype), t.flatten(),
                            None if s == None else s.flatten(),
                            class_labels=class_labels)
        #assert F_x.dtype == dtype
        return F_x
    ...
    def convert_to_fp32(self):
        """Convert the model to FP32 precision."""
        pass
