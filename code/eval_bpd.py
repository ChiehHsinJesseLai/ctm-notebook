...
def normalizing_Z(std_max, std_min, rho):
    """
    Calculate the normalizing constant Z for given standard deviations
    """
    return 2. * torch.log(std_max / std_min)

def get_importance_time(batch_size, std_max, std_min, rho):
    """
    Calculate the importance sampling time and normalizing constant Z.
    """
...
def gen_get_importance_time(batch, std_max, std_min, rho, t_min):
    """
    Generate the importance sampling time and normalizing constant Z for a given batch.
...
def sigma(t, std_max, std_min, rho):
    """
    Calculates a value based on the given time t, maximum and minimum 
    standard deviations and a parameter rho.
    """
    return std_min ** (1. / rho) + t * (std_max ** (1. / rho) - std_min ** (1. / rho))

def inv_sigma(sigma, std_max, std_min, rho):
    """
    Computes the inverse of the sigma function.
    """
    return (sigma - std_min ** (1. / rho)) / (std_max ** (1. / rho) - std_min ** (1. / rho))

def prior_logp(std_max, z):
    """
    Computes the log probability of the prior distribution for a given standard 
    deviation std_max and tensor z.
    """
    shape = z.shape
    N = np.prod(shape[1:])
    return (
    -N / 2. * torch.log(2 * np.pi * std_max ** 2)
    - torch.sum(z ** 2, dim=(1, 2, 3)) / (2 * std_max ** 2)
    )

def residual(
    _, model, diffusion, images,teacher=False,
    std_max=80., std_min=0.002,rho=7, t_min=1e-3):
    """
    Computes the residual for the given images using the diffusion model.
    """
    z = torch.randn_like(images)
    std = sigma(t_min, std_max, std_min, rho)
    perturbed_data = images + std[:, None, None, None] * z
    if teacher:
        denoised = diffusion.get_denoised_and_G(
        model, perturbed_data, std, s=std, ctm=False, teacher=True
        )[0].to(torch.float64)
    else:
        denoised = diffusion.get_denoised_and_G(
                    model, perturbed_data, std, s=std, ctm=True
                    )[0].to(torch.float64)
    ...
    q_recon = (
            n_dim / 2. * (np.log(2 * np.pi) + 2 * torch.log(q_std))
            + 0.5 / (q_std ** 2) * torch.square(images - q_mean).sum(axis=(1, 2, 3)))
    residual = q_recon - p_entropy
    return residual

def elbo(
    args, model, diffusion, images,
    teacher=False, std_max=80., std_min=0.002,
    rho=7, t_min=1e-3
    ):
    """
    Computes the Evidence Lower Bound (ELBO) for the given images using 
    the model and diffusion process.
    """
    ...
    with torch.enable_grad():
        ...
        if teacher:
            denoised = diffusion.get_denoised_and_G(
                       model,perturbed_data, std,
                       s=std,ctm=False,teacher=True
                       )[0].to(torch.float64)
        else:
            denoised = diffusion.get_denoised_and_G(
                       model,perturbed_data,std,
                       s=std,ctm=True)[0].to(torch.float64)
...
def drift_fn(model, diffusion, x, std_max, std_min, rho, t, teacher=False):
    ...
    if teacher:
        denoised = diffusion.get_denoised_and_G(model,x,std,s=std,
        ctm=False,teacher=True)[0].to(torch.float64)
    else:
...
def div_fn(model, diffusion, x, std_max, std_min, rho, t, noise, teacher=False):
    """
    Computes the divergence function for the given inputs using the model and diffusion process.
    """
    with torch.enable_grad():
        x.requires_grad_(True)  #
        fn_eps = torch.sum(
            drift_fn(model, diffusion, x, std_max, std_min, rho, t, teacher=teacher) * noise
        )
        grad_fn_eps = torch.autograd.grad(fn_eps, x)[0]
    x.requires_grad_(False)
    return torch.sum(grad_fn_eps * noise, dim=tuple(range(1, len(x.shape))))

def likelihood(args, model, diffusion, images,
                teacher=False, std_max=80.,
                std_min=0.002, rho=7, t_min=1e-3, mode='correct'):
    """
    Computes the likelihood of the given images using the model and diffusion process.
    """
    shape = images.shape
    noise = torch.randint_like(images, low=0, high=2).float() * 2 - 1.
    def ode_func(t, x):
        sample = torch.from_numpy(
            x[:-shape[0]].reshape(shape)
            ).to(images.device).type(torch.float32)
        vec_t = torch.ones(sample.shape[0], device=sample.device) * t
        drift = drift_fn(model, diffusion, sample, std_max,
                          std_min, rho, vec_t, teacher=teacher
                          ).detach().cpu().numpy().reshape((-1,))
        logp_grad = div_fn(model, diffusion, sample, std_max,
                            std_min, rho, vec_t, noise, teacher=teacher
                            ).detach().cpu().numpy().reshape((-1,))
    ...
    if mode == 'correct':
        ...
        perturbed_data = images + std[:, None, None, None] * z
        init = np.concatenate([perturbed_data.detach().cpu().numpy(
               ).reshape((-1,)), np.zeros((shape[0],))], axis=0)
    elif mode == 'wrong':
        init = np.concatenate([images.detach().cpu().numpy().reshape((-1,)
                ), np.zeros((shape[0],))], axis=0)
    else:
    ...
    delta_logp = torch.from_numpy(zp[-shape[0]:].reshape((shape[0],))
                 ).to(images.device).type(torch.float32)
    prior_logp_ = prior_logp(std_max, z)

    if mode == 'correct':
        residual_nll = residual(args, model, diffusion, images,
                       teacher, std_max=std_max, std_min=std_min)
...
def main():
    """
    Main function to set up the environment and configure the logger.
    """
    ...
    if args.batch_size == -1:
        ...
        if args.global_batch_size % dist.get_world_size() != 0:
            logger.log(
                ...
                f"{dist.get_world_size() * batch_size} instead of {args.global_batch_size}"
)
    else:
    ...
    for itr in range(args.num_student_elbo):
        ...
        for k in range(50000 // args.global_batch_size):
            ...
            elbo_ = elbo(args, model, diffusion,
                        batch, std_max=std_max,
                        std_min=std_min,
                        teacher = (args.training_mode != 'ctm')
                        ).cpu().detach().numpy()
    ...
    for itr in range(args.num_student_nll):
        ...
        for k in range(50000 // args.global_batch_size):
            batch,_ = next(data)
            batch = batch.to(dist_util.dev())
            likelihood_ = likelihood(args, model, diffusion, batch,
...
def create_argparser():
    """
    Create an argument parser with default values for training and model parameters.
