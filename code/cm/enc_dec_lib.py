...
def load_feature_extractor(args, eval=True):
    """
    Loads the feature extractor based on the specified loss normalization method.

    Args:
        args: Argument parser object containing various parameters.
        eval (bool): If True, sets the feature extractor to evaluation mode.

    Returns:
        feature_extractor: The initialized feature extractor.
    """
...
def load_discriminator_and_d_feature_extractor(args):
    ...
    """
    Loads the discriminator and its feature extractor based on the
    specified GAN training parameters.

    Args:
        args: Argument parser object containing various parameters.

    Returns:
        discriminator: The initialized discriminator.
        discriminator_feature_extractor: The initialized feature
        extractor for the discriminator.
    """
    if args.gan_training:
        ...
        print("cuda devices",dist_util.dev())
...
def gaussian_blur(args, img, step):
    """
    Applies Gaussian blur to the input image based on the specified parameters.
...
def get_feature(args, input, feat, brightness, saturation,
                contrast, translation_x, translation_y,
                offset_x, offset_y, name, step):
    """
    Extracts features from the input image after applying various augmentations.
    """
    # augment input
    input_aug_ = input
    if args.data_augment and brightness.shape[0] > 0:
        input_aug_ = DiffAugment(input[:brightness.shape[0]],
                                brightness, saturation, contrast, translation_x,
                                translation_y, offset_x, offset_y,
...
def get_xl_feature(args, estimate, target=None,
                feature_extractor=None,
                discriminator=None,
                step=-1, **model_kwargs):
    """
    Extracts features from the estimate and target images using the specified
    ...
    for bb_name, feat in feature_extractor.items():
        brightness = (torch.rand(int(estimate.size(0
                     ) * args.prob_aug), 1, 1, 1, dtype=estimate.dtype,
                    device=estimate.device) - 0.5)
        saturation = (torch.rand(int(estimate.size(0
                    ) * args.prob_aug), 1, 1, 1, dtype=estimate.dtype,
        ...
        translation_x = torch.randint(-shift_x, shift_x + 1,
                        size=[int(estimate.size(0) * args.prob_aug), 1, 1],
        ...
        offset_y = torch.randint(0, estimate.size(3) + (1 - cutout_size[1] % 2),
                            size=[int(estimate.size(0) * args.prob_aug), 1, 1],
                            device=estimate.device)
        estimate_feature = get_feature(args, estimate, feat, brightness,
                           saturation, contrast,
                           translation_x, translation_y,
                            offset_x, offset_y, 'estimate',step)
        ...
        if discriminator is not None:
            ...
            if args.gan_low_res_train:
                ...
                estimate_feature_low_res = get_feature(args, estimate_low_res,
                                           feat, brightness, saturation, contrast,
                                           translation_x, translation_y, offset_x,
                                           offset_y, 'estimate_low_res',step)
                logits_fake += discriminator.module[bb_name +
                               '_low'](estimate_feature_low_res, model_kwargs)

        if target != None:
            if args.gan_real_free and args.gan_different_augment:
                brightness = (torch.rand(int(target.size(0) * args.prob_aug),
                            1, 1, 1, dtype=target.dtype,
                            device=target.device) - 0.5)
                saturation = (torch.rand(int(target.size(0) * args.prob_aug),
                             1, 1, 1, dtype=target.dtype,
                            device=target.device) * 2)
                contrast = (torch.rand(int(target.size(0) * args.prob_aug),
                            1, 1, 1, dtype=target.dtype,
                            device=target.device) + 0.5)
                shift_x, shift_y = int(target.size(2) * args.shift_ratio + 0.5), int(
                    target.size(3) * args.shift_ratio + 0.5)
                translation_x = torch.randint(-shift_x, shift_x + 1, size=[int(
                                target.size(0) * args.prob_aug), 1, 1],
                                device=target.device)
                translation_y = torch.randint(-shift_y, shift_y + 1,
                                size=[int(target.size(0) * args.prob_aug), 1, 1],
                                device=target.device)
                ...
                offset_y = torch.randint(0, target.size(3) + (1 - cutout_size[1] % 2),
                                        ...
                                        device=target.device)
            with torch.no_grad():
                target_feature = get_feature(args, target, feat, brightness,
                                saturation, contrast,
                                translation_x, translation_y,
                                offset_x, offset_y, 'target',step)
                target_features.append(target_feature)
            if discriminator is not None:
                logits_real += discriminator.module[bb_name](target_feature,
                                                            model_kwargs)
                if args.gan_low_res_train:
                    target_low_res = F.interpolate(target, 16, mode='bilinear', align_corners=False)
                    target_feature_low_res = get_feature(args, target_low_res, feat, brightness,
                                                        saturation, contrast,
                                                        translation_x, translation_y,
                                                        offset_x, offset_y,
                                                        'target_low_res', step)
                    logits_real += discriminator.module[bb_name + '_low'](
                        target_feature_low_res, model_kwargs)
    if discriminator is not None:
        if target == None:
            return logits_fake
        else:
...
def get_classifier_guidance(classifier, vpsde,
                            unnormalized_input, std_wve_t,
                            img_resolution, class_labels,
                            log_prob=False):
    """
    Computes the classifier guidance for the given input.
    """
    mean_vp_tau, _, tau = vpsde.transform_unnormalized_wve_to_normalized_vp(std_wve_t,
                                                                    std_out=True)
    ...
    with torch.enable_grad():
        ...
        if log_prob:
            return selected
        classifier_guidance = torch.autograd.grad(outputs=selected.sum(),
                              inputs=x_, retain_graph=False)[0]
...
def get_log_ratio(discriminator, input, time, class_labels):
    """
    Computes the log ratio for the given input using the discriminator.

    Args:
        discriminator: The discriminator model.
        input: The input tensor.
        time: The current time step.
        class_labels: The class labels for the input.

    Returns:
        log_ratio: The computed log ratio.
    """
...
class vpsde():
    """
    Variance-Preserving Stochastic Differential Equation (VP-SDE) class.
    ...
    def compute_tau(self, std_wve_t, multiplier=-1.):
        """
        Computes the tau value for the given standard deviation of the wavelet transform.
        """
        if multiplier == -1:
            if self.multiplier == 1.:
                tau = -self.beta_0 + torch.sqrt(self.beta_0 ** 2 + 2. * (
                       self.beta_1 - self.beta_0) * torch.log(1. + std_wve_t ** 2))
                tau /= self.beta_1 - self.beta_0
            elif self.multiplier == 2.:
                d = - torch.log(1. + std_wve_t ** 2)
                in_ = (2 * (self.b ** 3) - 9 * self.a * self.b * self.c + 27. * (
                      self.a ** 2) * d) ** 2 - 4 * (((self.b ** 2) - 3 * self.a * self.c) ** 3)
                out_ = 2 * (self.b ** 3) - 9 * self.a * self.b * self.c + 27. * (self.a ** 2) * d
                plus = out_ + in_ ** 0.5
                minus =out_ - in_ ** 0.5
                sign_plus = torch.sign(plus)
                sign_minus = torch.sign(minus)
                tau = - self.b / (3. * self.a) - sign_plus * ((
                    torch.abs(plus) / 2.) ** (1/3.)) / (3. * self.a) - sign_minus * ((
                    torch.abs(minus) / 2.) ** (1/3.)) / (3. * self.a)
        elif multiplier == 1.:
            tau = -self.beta_0 + torch.sqrt(self.beta_0 ** 2 + 2. * (
                  self.beta_1 - self.beta_0) * torch.log(1. + std_wve_t ** 2))
            tau /= self.beta_1 - self.beta_0
    ...
    def marginal_prob(self, t, multiplier=-1.):
        """
        Computes the mean and standard deviation of the marginal probability distribution at time t.
        Args:
            t: The time variable.
            multiplier (float): A multiplier for the integral of beta.
        Returns:
            mean: The mean of the marginal probability distribution.
            std: The standard deviation of the marginal probability distribution.
        """
    ...
    def transform_normalized_vp_to_unnormalized_wve(self, t, multiplier=-1.):
        """
            Transforms normalized VP to unnormalized WVE.
        Args:
            t: The time variable.
            multiplier (float): A multiplier for the marginal probability.
        Returns:
            The transformed value from normalized VP to unnormalized WVE.
        """
    ...
    def sampling_std(self, num_step):
        """
        Computes the sampling standard deviation for a given number of steps.
        Args:
            num_step (int): The number of steps for sampling.
        Returns:
            std: The computed standard deviation for sampling.
        """
        assert 1000 % num_step == 0
        ddim_timesteps = torch.from_numpy(np.array(
                    list(range(0, 1000, 1000 // num_step)))[::-1].copy())
    ...
    def transform_unnormalized_wve_to_normalized_vp(self, t, std_out=False, multiplier=-1.):
        """
            Transforms unnormalized WVE to normalized VP.
        Args:
            t: The time variable.
            std_out (bool): If True, returns the standard deviation as well.
            multiplier (float): A multiplier for the marginal probability.
        Returns:
            mean_vp_tau: The mean of the normalized VP.
            std_vp_tau: The standard deviation of the normalized VP (if std_out is True).
            tau: The transformed time variable.
        """
    ...
    def from_rescaled_t_to_original_std(self, rescaled_t):
        """
        Converts rescaled time to the original standard deviation.
        Args:
            rescaled_t: The rescaled time variable.
        Returns:
    ...
    def compute_t_cos_from_t_lin(self, t_lin):
        """
            Computes the cosine time from the linear time.
        Args:
            t_lin: The linear time variable.
        Returns:
            t_cos: The cosine time variable.
        """
        sqrt_alpha_t_bar = torch.exp(-0.25 * t_lin ** 2 * (
            self.beta_1 - self.beta_0) - 0.5 * t_lin * self.beta_0)
    ...
    def get_diffusion_time(self, batch_size, batch_device, t_min=1e-5, importance_sampling=True):
        """
        Computes the diffusion time for a batch of samples.
        Args:
            batch_size (int): The size of the batch.
            batch_device: The device on which the batch is located.
            t_min (float): The minimum time value.
            importance_sampling (bool): If True, uses importance sampling.
        Returns:
            The diffusion time and the normalizing constant.
        """
        if importance_sampling:
            Z = self.normalizing_constant(t_min)
            u = torch.rand(batch_size, device=batch_device)
            return (-self.beta_0 + torch.sqrt(self.beta_0 ** 2 + 2 * (
                self.beta_1 - self.beta_0) *
                torch.log(1. + torch.exp(Z * u + self.antiderivative(
                t_min))))) / (self.beta_1 - self.beta_0), Z.detach()
        else:
            return torch.rand(batch_size, device=batch_device) * (self.T - t_min) + t_min, 1

    def antiderivative(self, t, stabilizing_constant=0.):
        """
            Computes the antiderivative of the integral of beta.
        Args:
            t: The time variable.
            stabilizing_constant (float): A stabilizing constant for numerical stability.
        Returns:
            The computed antiderivative.
        """
        if isinstance(t, float) or isinstance(t, int):
            t = torch.tensor(t).float()
        return torch.log(1. - torch.exp(- self.integral_beta(t
                )) + stabilizing_constant) + self.integral_beta(t)
    ...
    def integral_beta(self, t, multiplier=-1.):
        """
        Computes the integral of beta over time.
        Args:
            t: The time variable.
            multiplier (float): A multiplier for the integral of beta.
        Returns:
            The computed integral of beta.
        """
        if multiplier == -1.:
            if self.multiplier == 1.:
                ...
                return ((self.beta_1 ** 0.5 - self.beta_0 ** 0.5) ** 2) * (t ** 3) / 3. \
                      + (self.beta_0 ** 0.5) * (
                        self.beta_1 ** 0.5 - self.beta_0 ** 0.5) * (
                        t ** 2) + self.beta_0 * t
        elif multiplier == 1.:
            return 0.5 * t ** 2 * (self.beta_1 - self.beta_0) + t * self.beta_0
        elif multiplier == 2.:
            return ((self.beta_1 ** 0.5 - self.beta_0 ** 0.5) ** 2) * (t ** 3) / 3. \
                + (self.beta_0 ** 0.5) * (
                self.beta_1 ** 0.5 - self.beta_0 ** 0.5) * (t ** 2) + self.beta_0 * t
