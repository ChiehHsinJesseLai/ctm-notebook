...
def _conv2d_wrapper(x,
                    ...
                    stride=1,
                    padding=0,
                    groups=1, transpose=False, flip_weight=True):
...
def conv2d_resample(x,
    ...
    assert f is None or (isinstance(f,
                                    torch.Tensor) and f.ndim in [1, 2] and f.dtype == torch.float32)
