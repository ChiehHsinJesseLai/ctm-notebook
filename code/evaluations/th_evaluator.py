...
def clip_preproc(preproc_fn, x):
    """
     Apply a preprocessing function to an image array.
    """
...
class FIDStatistics:
    """
    Class to hold statistics for Frechet Inception Distance (FID) calculation.
    """
    def __init__(self, mu: np.ndarray, sigma: np.ndarray, resolution: int):
        """
        Initialize FIDStatistics with mean (mu), covariance (sigma), and resolution.
        """
        self.mu = mu
        self.sigma = sigma
...
class FIDAndIS:
    """
    Class to compute Frechet Inception Distance (FID) and Inception Score (IS) using InceptionV3 and CLIP models.
    """
    def __init__(
        self,
        ...
        """
        Initialize the FIDAndIS class with batch sizes and model path.
        """
        import clip
        ...
        class ProjLayer(nn.Module):
            """
            Projection layer to compute probe features separately from the final projection
            """
            def __init__(self, param):
                """
                Initialize the ProjLayer with the given parameter.
                """
                super().__init__()
                self.param = param
        ...
        class TextModel(nn.Module):
            """
            Text model for encoding text using the CLIP model.
            """
            def __init__(self, clip_model):
                """
                Initialize the TextModel with the given CLIP model.
                """
                super().__init__()
            ...
            def forward(self, x):
                """
                Forward pass through the TextModel to encode text.
                """
    ...
    def get_preds(self, batch, captions=None):
        """
        Get predictions from the InceptionV3 and CLIP models for a batch of images.
        """
        with torch.no_grad():
            batch = 127.5 * (batch + 1)
    ...
    def get_clip_score(
        ...
        """
        Compute the CLIP score using image and text features.
        """
    ...
    def get_activations(self, data, num_samples, global_batch_size, pr_samples=50000):
        """
        Get activations from the InceptionV3 and CLIP models for a dataset.
        """
        if self.is_root:
            preds = []
            spatial_preds = []
    ...
    def get_virtual_batch(self, data, num_samples, global_batch_size, resolution):
        """
        Get a virtual batch of activations and compute statistics.
        """
    ...
    def set_ref_batch(self, ref_batch):
        """
        Set the reference batch statistics from a file.
        """
    ...
    def get_ref_batch(self, ref_batch):
        """
        Get the reference batch statistics.
        """
