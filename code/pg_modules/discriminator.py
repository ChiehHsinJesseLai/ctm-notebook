...
from pg_modules.networks_stylegan2 import FullyConnectedLayer
...
class SingleDisc(nn.Module):
    """
    SingleDisc class for a single discriminator network.
    """
    ...
    def forward(self, x, c):
        """
        Forward pass for SingleDisc.
        """
        return self.main(x)

class SingleDiscCond(nn.Module):
    """
    SingleDiscCond class for a conditional single discriminator network.
    """
    def __init__(self, nc=None, ndf=None, start_sz=256, end_sz=8, head=None,
                 patch=False, c_dim=1000, cmap_dim=64, rand_embedding=False):
        """
        Initialization code
        """
        super().__init__()
        ...
        if rand_embedding:
            self.embed.__init__(num_embeddings=self.embed.num_embeddings,
                                embedding_dim=self.embed.embedding_dim)
            print(f'initialized embeddings with random weights')
        self.embed_proj = FullyConnectedLayer(self.embed.embedding_dim,
                                              self.cmap_dim, activation='lrelu')

    def forward(self, x, c):
        """
        Forward pass for SingleDiscCond.
        """
        h = self.main(x)
        out = self.cls(h)
...
class MultiScaleD(nn.Module):
    """
    MultiScaleD class for a multi-scale discriminator network.
    """
    def __init__(
        self,
        channels,
        ...
        **kwargs,
        ...
        for i, (cin, res) in enumerate(zip(self.disc_in_channels, self.disc_in_res)):
            ...
            mini_discs.append((str(i), Disc(nc=cin, start_sz=start_sz, end_sz=8, patch=patch)))

        self.mini_discs = nn.ModuleDict(mini_discs)

    def forward(self, features, c, rec=False):
        """
        Forward pass for MultiScaleD.
        """
        all_logits = []
...
class ProjectedDiscriminator(torch.nn.Module):
    """
    ProjectedDiscriminator class for a projected discriminator network.
    """
    def __init__(
        self,
        backbones,
    ...
    def train(self, mode=True):
        """
        Sets the training mode for the discriminators and 
        evaluation mode for the feature networks.
        """
    ...
    def forward(self, x, c):
        """
        Forward pass for ProjectedDiscriminator.
        """
        logits = []
